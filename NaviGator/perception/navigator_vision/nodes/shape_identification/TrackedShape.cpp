...
TrackedShape::TrackedShape(navigator_msgs::DockShape& s) : count(1), latest(s)
{
}
...
{
  return (color == navigator_msgs::GetDockShape::Request::ANY || color == latest.Color) &&
         (shape == navigator_msgs::GetDockShape::Request::ANY || shape == latest.Shape);
}
