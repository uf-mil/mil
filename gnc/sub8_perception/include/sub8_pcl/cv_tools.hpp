...
cv::MatND smooth_histogram(const cv::MatND &histogram, size_t filter_kernel_size = 3,
                           float sigma = 1.0);

// Generate a one-dimensional gaussian kernel given a kernel size and it's standard deviation
// (sigma)
...
struct ImageWithCameraInfo {
  /**
          Packages corresponding  sensor_msgs::ImageConstPtr and sensor_msgs::CameraInfoConstPtr
     info_msg
