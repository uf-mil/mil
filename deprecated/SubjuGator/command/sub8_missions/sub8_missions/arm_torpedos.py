...
class FireTorpedos(object):
    ...
    def fire(self, target):
        ...
        target_position = transform._q_mat.dot(target_pose - transform._p)
        ...
        yield self.sub.move.relative(np.array([0, target_position[1], 0])).go(
            blind=True, speed=0.1
        )
        yield self.sub.move.relative(
            np.array(
                [
                    target_position[0] + self.X_OFFSET,
                    0,
                    target_position[2] + self.Z_OFFSET,
                ]
            )
        ).go(blind=self.BLIND, speed=0.1)
