...
__all__ = [
    "identity_fields",
    "text_search_fields",
    "extent_search_fields",
    "other_extent_fields",
    "creator_search_fields",
    "contributor_search_fields",
...
identity_fields = [
    {
        "Conventions": "Dataset conventions. E.g. CF-1.6, ISO 19115-2, COARDS",
    },
    {
...
other_extent_fields = [
    {},
    {
        "geospatial_bounds": "The north, south, east, west bounding box",
...
creator_search_fields = [
    {},
    {
        "acknowledgment": "acknowledge various type of support for the project that produced this data",
...
contributor_search_fields = [
    ...
    {},
    {
        "contributor_name": "Enter a string here...",
        "contributor_role": "Enter a string here...",
    },
]
publisher_search_fields = [
    {},
    {
        "publisher_name": "The data publisher's name. Can be an individual or an institution",
...
instrument_fields = [
    ...
    {},
    {
        "camera_exposure_time": "Exposure time for each camera frame in ms",
...
def build_section_widgets(
    section_name: str = "", fields: List[dict] = [{}], cols: int = 3
) -> "panel of text widgets":
    ...
    for i in range(len(fields)):
        ...
        if i == 0 and len(fields[i]) > 0:
            wgts_layout.append(
                pn.Column(pn.pane.Markdown("*Highly Recommended:*"), pn.Row(*temp))
            )
        elif i == 1 and len(fields[i]) > 0:
            wgts_layout.append(
                pn.Column(
                    pn.pane.Markdown("*Recommended:*"), pn.GridBox(*temp, ncols=cols)
                )
            )
        elif i == 2 and len(fields[i]) > 0:
            wgts_layout.append(
                pn.Column(
                    pn.pane.Markdown("*Suggested:*"), pn.GridBox(*temp, ncols=cols)
                )
            )
...
def build_variables_widgets(ds: xr.Dataset, cols: int = 3):
    ...
    for n in coord_names:
        ...
        if "time" not in n:
            temp.append(
                pnw.input.TextAreaInput(
                    name="units",
                    placeholder="Units for data variable",
                    value=ds[n].attrs.get("units"),
                    width=150,
                )
            )
            temp.append(
                pnw.input.TextAreaInput(
                    ...
                    placeholder="ISO 19115-1 code (image, thematicClassification, physicalMeasurement, auxiliaryInformation, qualityInformation, referenceInformation, modelResult, or coordinate)",
                    value=ds[n].attrs.get("coverage_content_type"),
                    width=550,
                )
            )
        temp.append(
            pnw.input.TextAreaInput(
                name="long_name",
                placeholder="Long descriptive name for the variable (not necessarily from a controlled vocabulary)",
                value=ds[n].attrs.get("long_name"),
                width=150,
            )
        )
        temp.append(
            pnw.input.TextAreaInput(
                name="standard_name",
                placeholder="Long descriptive name for the variable taken from a controlled vocabulary of variable names",
                value=ds[n].attrs.get("standard_name"),
            )
        )
        wgts.extend(temp)

        wgts_layout.append(
            pn.Column(pn.pane.Markdown(f"*{n}:*"), pn.GridBox(*temp, ncols=cols))
        )
...
class MetadataEditor:
    ...
    def __init__(self):
        ...
        self.file_path_box = pnw.input.TextAreaInput(
            name="File path:", placeholder="Enter path string here...", value=""
        )
        self.extract_button = pnw.Button(
            name="Extract Metadata", button_type="primary", width=150, height=25
        )
        self.update_button = pnw.Button(
            name="Update Metadata", button_type="primary", width=150, height=25
        )
        self.export_button = pnw.Button(
            name="Export to JSON", button_type="primary", width=150, height=25
        )
    ...
    def __call__(self) -> "dashboard":
        ...
        return pn.Column(
            ...
            pn.Row(
                self.file_path_box,
                pn.Column(self.extract_button, self.update_button, self.export_button),
                self.event_msg,
            ),
            self.metadata_tool,
        )
        ...
        def click_func(event):
            try:
                ...
                for w_list in [
                    ...
                    self.text_search_values,
                    self.extent_search_values,
                    self.other_extent_values,
                    self.creator_search_values,
                    self.contributor_search_values,
                    self.publisher_search_values,
                    self.other_values,
                ...
                with open(
                    ".".join(self.file_path_box.value.split(".")[:-1]) + ".json", "w"
                ) as outfile:
                    json.dump(
                        self.attrs,
                        outfile,
                ...
                self.event_msg.value = (
                    f"Metadata exported {self.export_button.clicks} time(s)"
                )
    ...
    def update_extract(self):
        ...
        def click_func(event):
            try:
                ...
                self.event_msg.value = (
                    f"Metadata extracted {self.extract_button.clicks} time(s)"
                )
                ...
                if self.extracted is False:
                    ...
                    ) = build_variables_widgets(self.ds, cols=2)
    ...
    def update_save(self):
        ...
        def click_func(event):
            try:
                ...
                coord_names = [
                    s.split("(")[0].strip() for s in str(self.ds.coords).split("*")[1:]
                ]
                coord_dict = {c: self.ds[c] for c in coord_names}
                coord_names.append("datacube")
                ...
                for n in coord_names:
                    if "time" not in n:
                        if self.variables_values[w_count].value is not None:
                            self.ds[n].attrs.update(
                                {
                                    self.variables_values[w_count]
                                    .name: self.variables_values[w_count]
                                    .value
                                }
                            )
                        if self.variables_values[w_count + 1].value is not None:
                            self.ds[n].attrs.update(
                                {
                                    self.variables_values[w_count + 1]
                                    .name: self.variables_values[w_count + 1]
                                    .value
                                }
                            )
                        w_count += 2
    ...
    def make_widgets(self):
        """init widgets!"""
        self.identity_wgts, self.identity_values = build_section_widgets(
            "Identification / Metadata Reference", identity_fields
        )
        self.text_search_wgts, self.text_search_values = build_section_widgets(
            "Text Search", text_search_fields
        )
        self.extent_search_wgts, self.extent_search_values = build_section_widgets(
            "Extent Search", extent_search_fields
        )
        self.other_extent_wgts, self.other_extent_values = build_section_widgets(
            "Other Extent Information", other_extent_fields
        )
        self.creator_search_wgts, self.creator_search_values = build_section_widgets(
            "Creator Search", creator_search_fields
        )
        (
            self.contributor_search_wgts,
            self.contributor_search_values,
        ) = build_section_widgets("Contributor Search", contributor_search_fields)
        (
            self.publisher_search_wgts,
            self.publisher_search_values,
        ) = build_section_widgets("Publisher Search", publisher_search_fields)
        self.other_wgts, self.other_values = build_section_widgets(
            "Other Attributes", other_fields
        )
        self.instrument_wgts, self.instrument_values = build_section_widgets(
            "Instrument Attributes", instrument_fields
        )

        self.metadata_tabs = pn.Tabs(
            ("Identification Reference", self.identity_wgts),
