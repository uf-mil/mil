            ...
            while not rospy.is_shutdown() and times_ran < self.number_of_iterations:
                ...
                for message in split_rx_data:
                    ...
                    if data_list[0] == "$RXGAT":
                        ...
                        final_checksum_string = hex_checksum + "\r\n"
                        self.assertEquals(
                            len(data_list), 8, "entrance exit gate message formatting incorrect")
                        if self.use_test_data is True:
                            ...
                            self.assertEquals(int(data_list[5]), int(
                                list_test_data[5]), "exit gate incorrect")
                            self.assertEquals(
                                data_list[6], list_test_data[6], "light buoy boolean incorrect")
                            msg_light_pattern = data_list[7].split("*")[0]
                            light_pattern = list_test_data[7].split("*")[0]
                            self.assertEquals(
                                msg_light_pattern, light_pattern, "light pattern incorrect")
                        else:
                            self.assertEquals(
                ...
                for message in split_rx_data:
                    ...
                    if data_list[0] == "$RXCOD":
                        ...
                        if self.use_test_data is True:
                            ...
                            self.assertEquals(
                                msg_color_pattern, light_pattern, "light pattern incorrect")
                        else:
                            self.assertEquals(
                                data_list[3], self.team_id, "team id incorrect")
                            self.assertEquals(
    ...
    def test_identify_symbols_dock_message(self):
        ...
        try:
            while not rospy.is_shutdown() and times_ran < self.number_of_iterations:
                ...
                split_rx_data = rx_data.splitlines(True)
                for message in split_rx_data:
                    deserialized_msg = robot_x_identify_symbols_dock_message.from_string(
                        self.delim, message)
                    data_list = deserialized_msg[0]
                    checksum_list = deserialized_msg[1]
                    if data_list[0] == "$RXDOK":
                        ...
                        final_checksum_string = hex_checksum + "\r\n"
                        self.assertEquals(
                            len(data_list), 6, "identify symbols dock message formatting incorrect")
                        if self.use_test_data is True:
                            ...
                            self.assertEquals(
                                msg_shape, shape, "shape incorrect")
                        else:
                            self.assertEquals(
                                data_list[3], self.team_id, "team id incorrect")
                            self.assertEquals(
                            ...
                            msg_shape = data_list[5].split("*")[0]
                            self.assertEquals(
                                shape, msg_shape, "shape incorrect")
                        times_ran += 1
        finally:
            self.server.disconnect()
            ...
            while not rospy.is_shutdown() and times_ran < self.number_of_iterations:
                ...
                for message in split_rx_data:
                    ...
                    if data_list[0] == "$RXDEL":
                        ...
                        final_checksum_string = hex_checksum + "\r\n"
                        self.assertEquals(
                            len(data_list), 6, "detect deliver message formatting incorrect")
                        if self.use_test_data is True:
