...
PerceptionModel::PerceptionModel(std::vector<float> model_params,
                                 int min_points)
    : width(model_params[0]), height(model_params[1]), min_points(min_points) {
...
bool PerceptionModel::check_point(Eigen::Vector3d point, cv::Point point2d,
                                  cv::Mat img, cv::Matx34d left_cam_mat,
                                  bool check)

{
  ...
  if (check) {
    ...
    for (size_t i = 0; i < current_points.size(); i++) {
      ...
      cv::Point2d L_center2d(pt_L_2d_hom(0) / pt_L_2d_hom(2),
                             pt_L_2d_hom(1) / pt_L_2d_hom(2));
      cv::Scalar color(255, 0, 255);
      std::stringstream label;
      label << i;
      cv::circle(current_image_left, L_center2d, 5, color, -1);
      cv::putText(current_image_left, label.str(), L_center2d,
                  cv::FONT_HERSHEY_SIMPLEX, 0.0015 * current_image_left.rows,
                  cv::Scalar(0, 0, 0), 2);
    }
  ...
  for (float dist : unused_distances) {
    ...
    // if the point is within some range of the current distance, then it's a
    // point that fits the current model
    if (dist_from_starting > lower && dist_from_starting < upper) {
      ...
      // maps the point to the distance, so later it can be added back to unused
      // distances
      point_to_distance.insert(std::pair<std::string, float>(ss.str(), dist));
      // If this is the final point in our model, add the full model to the list
      // of potential models
  ...
  // If the point fit the model, remove its corresponding distance from list of
  // unused distances
  if (point_fits_model) {
    // unused_distances.erase(std::remove(unused_distances.begin(),
    // unused_distances.end(), minel), unused_distances.end());
  }
...
void PerceptionModel::remove_point(Eigen::Vector3d point, cv::Point point2d) {
  current_points.erase(
      std::remove(current_points.begin(), current_points.end(), point),
      current_points.end());
  current_points_2d.erase(
      std::remove(current_points_2d.begin(), current_points_2d.end(), point2d),
      current_points_2d.end());
...
void PerceptionModel::visualize_points(
    std::vector<Eigen::Vector3d> feature_pts_3d, cv::Mat img,
    cv::Matx34d left_cam_mat, std::string name, double min_cost) {
  cv::Mat current_image_left = img.clone();
  ...
  //    std::stringstream label;
  //    label << min_cost;
  //    cv::putText(current_image_left, label.str(), cv::Point2d(10,10),
...
bool PerceptionModel::get_model(std::vector<Eigen::Vector3d> &model3d,
                                std::vector<cv::Point> &model_position_2d,
                                cv::Mat left_image, cv::Matx34d left_cam_mat) {
  ...
  for (int i = 0; i != potential_models.size(); ++i) {
    ...
    for (int i = 0; i != min_points; ++i) {
      ...
      // Select the other to points, other than the current and the furthest
      // away
      ...
      double h = e.dot(f);
      double t1 = acos(h);
      // The farther away from 90 degrees the points are the larger the cost of
      // this model
...
int PerceptionModel::get_furthest_point(std::vector<Eigen::Vector3d> mymodel,
                                        int point) {
  float max_val = 0;
  int max = -1;
  for (int j = 0; j != 4; ++j) {
