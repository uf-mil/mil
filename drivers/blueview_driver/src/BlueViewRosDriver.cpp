...
BlueViewRosDriver::BlueViewRosDriver() :
...
void BlueViewRosDriver::initParams()
{
	...
	if (do_raw) {
		ping_msg.reset(new sonar::BlueViewPing);
		raw_pub = nh.advertise<sonar::BlueViewPing>("ranges", 5);
	}
	...
	else throw std::runtime_error("Can not connect: neither 'file' or 'device' param set");

	// Set Ranges, meters
	BVTSDK::Head& head  = sonar.getHead();
	float range_lower, range_upper;
	if (nh.getParam("range/start", range_lower)) head.SetStartRange(range_lower);
	if (nh.getParam("range/stop",  range_upper)) head.SetStopRange(range_upper);

	// Handle fluid type, enum string
	std::string fluid_type;
	if (nh.getParam("fluid_type", fluid_type)) {
		if (fluid_type == "saltwater") head.SetFluidType(BVTSDK::FluidType::Saltwater);
		else if (fluid_type == "freshwater") head.SetFluidType(BVTSDK::FluidType::Freshwater);
		else if (fluid_type == "other") head.SetFluidType(BVTSDK::FluidType::Other);
		else ROS_ERROR("Fluid type (%s) invalid, must be 'saltwater', 'freshwater', or 'other'", fluid_type.c_str());
	}

	// Set sound speed, m/s
	int sound_speed;
	if (nh.getParam("sound_speed", sound_speed)) head.SetSoundSpeed(sound_speed);
	
	// Set analog gain adjustment in dB
	float gain;
	if (nh.getParam("gain_adjustment", gain)) head.SetGainAdjustment(gain);
	
	// Set "time variable analog gain", in dB/meter,
	float tvg;
	if (nh.getParam("tvg_slope", tvg)) head.SetTVGSlope(tvg);
	
	// Set dynamic power managment
	bool dynamic_power;
	if (nh.getParam("dynamic_power_management", dynamic_power)) head.SetDynamicPowerManagement(dynamic_power);
	
	// Set ping interval
	float ping_interval;
	if (nh.getParam("ping_interval", ping_interval)) head.SetPingInterval(ping_interval);
...
int main(int argc, char** argv)
{
	...
	ros::spin();
		...
		printf("Exception thrown in Blue View SDK (Error #%d):\n\t%s: %s\n", err.ReturnCode(), err.ErrorName().c_str() ,err.ErrorMessage().c_str());
	}
}
