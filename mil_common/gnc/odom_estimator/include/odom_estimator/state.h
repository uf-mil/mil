...
{
ODOM_ESTIMATOR_DEFINE_MANIFOLD_BEGIN(State, (ros::Time, t)(ros::Time, t_start),
                                     (Vec<3>, pos_eci)(Vec<3>, rel_pos_eci)(QuaternionManifold, orient)(Vec<3>, vel)(
                                         Vec<3>, gyro_bias)(Vec<3>, accel_bias))
...
Vec<3> getVelECI(Vec<3> body_point = Vec<3>::Zero(), boost::optional<Vec<3> > gyro = boost::none) const
{
  Vec<3> result = vel;
  if (body_point != Vec<3>::Zero())
  {
    assert(gyro);
    result += orient._transformVector((*gyro - gyro_bias).cross(body_point));
  }
  return result;
}
Vec<3> getVelECEF(Vec<3> body_point = Vec<3>::Zero(), boost::optional<Vec<3> > gyro = boost::none) const
{
  return ecef_vel_from_inertial_vel(t.toSec(), getVelECI(body_point, gyro), pos_eci);
}
...
ODOM_ESTIMATOR_DEFINE_MANIFOLD_BEGIN(_PredictNoise, ,
                                     (Vec<3>, gyro)(Vec<3>, accel)(Vec<3>, gyro_bias_noise)(Vec<3>, accel_bias_noise))
ODOM_ESTIMATOR_DEFINE_MANIFOLD_END()
class StateUpdater : public UnscentedTransformDistributionFunction<State, State, _PredictNoise>
{
  ...
  GaussianDistribution<_PredictNoise> get_extra_distribution() const
  {
    return GaussianDistribution<_PredictNoise>(
        _PredictNoise(xyz2vec(imu.angular_velocity), xyz2vec(imu.linear_acceleration), Vec<3>::Zero(), Vec<3>::Zero()),
        joinDiagonally(joinDiagonally(Eigen::Map<const SqMat<3> >(imu.angular_velocity_covariance.data()),
                                      Eigen::Map<const SqMat<3> >(imu.linear_acceleration_covariance.data())),
                       joinDiagonally((Vec<3>::Ones() * pow(1e-3, 2)).asDiagonal(),
  ...
  State apply(State const &state, _PredictNoise const &extra) const
  {
    ...
    Vec<3> accelnograv_world = world_from_accelbody._transformVector(accelnograv_accelbody);
    Vec<3> accel_world = accelnograv_world + gravity::gravity(state.pos_eci);

    return State(imu.header.stamp, state.t_start, state.pos_eci + dt * state.vel + dt * dt / 2 * accel_world,
                 state.rel_pos_eci + dt * state.vel + dt * dt / 2 * accel_world, world_from_newbody,
                 state.vel + dt * accel_world, state.gyro_bias + sqrt(dt) * extra.gyro_bias_noise,
