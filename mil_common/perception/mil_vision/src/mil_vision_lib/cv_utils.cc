...
std::vector<cv::Point> find_local_maxima(const cv::MatND &histogram, float thresh_multiplier)
{
  ...
  for (size_t i = 0; i < local_maxima.size(); i++)
  {
    ...
    if (i % 4 == 0)
      ros_log << std::endl << "\t";
    ros_log << "[" << std::setw(5) << local_maxima[i].x << "," << std::setw(5) << local_maxima[i].y << "] ";
  }
  ros_log << std::endl
          << "thresh: > global_maximum(" << global_maximum << ") * thresh_multiplier(" << thresh_multiplier
          << ") = " << (global_maximum * thresh_multiplier);
  ros_log << std::endl << "Threshed Maxima (x): ";
...
std::vector<cv::Point> find_local_minima(const cv::MatND &histogram, float thresh_multiplier)
{
  ...
  for (size_t i = 0; i < local_minima.size(); i++)
  {
    ...
    if (i % 4 == 0)
      ros_log << std::endl << "\t";
    ros_log << "[" << std::setw(5) << local_minima[i].x << "," << std::setw(5) << local_minima[i].y << "] ";
  }
  ros_log << std::endl
          << "thresh: < global_minimum(" << global_minimum << ") * thresh_multiplier(" << thresh_multiplier
          << ") = " << (global_minimum * thresh_multiplier);
  ros_log << std::endl << "Threshed Minima (x): ";
...
void statistical_image_segmentation(const cv::Mat &src, cv::Mat &dest, cv::Mat &debug_img, const int hist_size,
                                    const float **ranges, const int target, std::string image_name, bool ret_dbg_img,
                                    const float sigma, const float low_thresh_gain, const float high_thresh_gain)
{
  ...
  // Find target mode
  std::vector<cv::Point> histogram_modes = mil_vision::find_local_maxima(hist_smooth, 0.1);
  ...
  std::vector<cv::Point> derivative_maxima = mil_vision::find_local_maxima(hist_derivative, 0.01);
  std::vector<cv::Point> derivative_minima = mil_vision::find_local_minima(hist_derivative, 0.01);
  ...
  ros_log << "Looking for the local minimum of the derivative of the histogram "
             "immediately to the right of the selected mode."
          << std::endl;

  for (size_t i = 0; i < derivative_minima.size(); i++)
  {
    ros_log << "\tderivative_minima[" << i << "].x = " << derivative_minima[i].x << std::endl;
    if (derivative_minima[i].x > target_mode)
    {
      high_thresh_search_start = derivative_minima[i].x;
      ros_log << "Done: The upper threshold will be no less than " << high_thresh_search_start << std::endl;
      break;
    }
  }
  ...
  ros_log << "Looking for the local maximum of the derivative of the histogram "
             "immediately to the left of the selected mode."
          << std::endl;
  for (int i = derivative_maxima.size() - 1; i >= 0; i--)
  {
    ros_log << "\tderivative_maxima[" << i << "].x = " << derivative_maxima[i].x << std::endl;
    if (derivative_maxima[i].x < target_mode)
    {
      low_thresh_search_start = derivative_maxima[i].x;
      ros_log << "Done: The lower threshold will be no greater than " << low_thresh_search_start << std::endl;
      break;
    }
  }
  int high_thresh = high_thresh_search_start;
  int low_thresh = low_thresh_search_start;
  ros_log << "high_deriv_thresh: " << hist_deriv_stddev[0] << " * " << high_thresh_gain << " = "
          << high_abs_derivative_thresh << std::endl;
  ...
  for (int i = high_thresh_search_start; i < hist_size; i++)
  {
    ...
    ros_log << "\ti = " << i << "  :  " << high_abs_derivative_thresh << " - " << abs_slope << " = "
            << high_abs_derivative_thresh - abs_slope << std::endl;
  ...
  ros_log << "low_deriv_thresh: " << hist_deriv_stddev[0] << " * " << low_thresh_gain << " = "
          << low_abs_derivative_thresh << std::endl;
  ...
  {
    int abs_slope = std::abs(hist_derivative.at<float>(i));
    ros_log << "\ti = " << i << "  :  " << low_abs_derivative_thresh << " - " << abs_slope << " = "
            << high_abs_derivative_thresh - abs_slope << std::endl;
  ...
  ros_log << "\x1b[1;37mTarget: " << target << "\nClosest distribution mode: " << target_mode
          << "\nThresholds selected:  low=" << low_thresh << "  high=" << high_thresh << "\x1b[0m" << std::endl;
    ...
    try
    {
      ...
      for (int i = 1; i < hist_size; i++)
      {
        ...
        cv::line(histImage, cv::Point(bin_w * (i - 1), hist_h - cvRound(hist_smooth.at<float>(i - 1))),
                 cv::Point(bin_w * (i), hist_h - cvRound(hist_smooth.at<float>(i))), cv::Scalar(255, 0, 0), 2, 8, 0);
        // Plot image histogram derivative
        cv::line(histDerivImage, cv::Point(bin_w * (i - 1), hist_h - cvRound(hist_derivative.at<float>(i - 1))),
                 cv::Point(bin_w * (i), hist_h - cvRound(hist_derivative.at<float>(i))), cv::Scalar(122, 0, 0), 1, 8,
                 0);
      }
      ...
      cv::line(histImage, cv::Point(bin_w * low_thresh, hist_h - cvRound(hist_smooth.at<float>(low_thresh))),
               cv::Point(bin_w * low_thresh, hist_h), cv::Scalar(255, 255, 0), 2);
      cv::line(histImage, cv::Point(bin_w * high_thresh, hist_h - cvRound(hist_smooth.at<float>(high_thresh))),
               cv::Point(bin_w * high_thresh, hist_h), cv::Scalar(255, 255, 0), 2);
      cv::floodFill(histImage, cv::Point(bin_w * cvRound(float(low_thresh + high_thresh) / 2.0), hist_h - 1),
                    cv::Scalar(155));
...
cv::Mat triangulate_Linear_LS(cv::Mat mat_P_l, cv::Mat mat_P_r, cv::Mat undistorted_l, cv::Mat undistorted_r)
{
  ...
  W.at<double>(0, 0) = 1.0;
  A.at<double>(0, 0) = (undistorted_l.at<double>(0, 0) / undistorted_l.at<double>(2, 0)) * mat_P_l.at<double>(2, 0) -
                       mat_P_l.at<double>(0, 0);
  A.at<double>(0, 1) = (undistorted_l.at<double>(0, 0) / undistorted_l.at<double>(2, 0)) * mat_P_l.at<double>(2, 1) -
                       mat_P_l.at<double>(0, 1);
  A.at<double>(0, 2) = (undistorted_l.at<double>(0, 0) / undistorted_l.at<double>(2, 0)) * mat_P_l.at<double>(2, 2) -
                       mat_P_l.at<double>(0, 2);
  A.at<double>(1, 0) = (undistorted_l.at<double>(1, 0) / undistorted_l.at<double>(2, 0)) * mat_P_l.at<double>(2, 0) -
                       mat_P_l.at<double>(1, 0);
  A.at<double>(1, 1) = (undistorted_l.at<double>(1, 0) / undistorted_l.at<double>(2, 0)) * mat_P_l.at<double>(2, 1) -
                       mat_P_l.at<double>(1, 1);
  A.at<double>(1, 2) = (undistorted_l.at<double>(1, 0) / undistorted_l.at<double>(2, 0)) * mat_P_l.at<double>(2, 2) -
                       mat_P_l.at<double>(1, 2);
  A.at<double>(2, 0) = (undistorted_r.at<double>(0, 0) / undistorted_r.at<double>(2, 0)) * mat_P_r.at<double>(2, 0) -
                       mat_P_r.at<double>(0, 0);
  A.at<double>(2, 1) = (undistorted_r.at<double>(0, 0) / undistorted_r.at<double>(2, 0)) * mat_P_r.at<double>(2, 1) -
                       mat_P_r.at<double>(0, 1);
  A.at<double>(2, 2) = (undistorted_r.at<double>(0, 0) / undistorted_r.at<double>(2, 0)) * mat_P_r.at<double>(2, 2) -
                       mat_P_r.at<double>(0, 2);
  A.at<double>(3, 0) = (undistorted_r.at<double>(1, 0) / undistorted_r.at<double>(2, 0)) * mat_P_r.at<double>(2, 0) -
                       mat_P_r.at<double>(1, 0);
  A.at<double>(3, 1) = (undistorted_r.at<double>(1, 0) / undistorted_r.at<double>(2, 0)) * mat_P_r.at<double>(2, 1) -
                       mat_P_r.at<double>(1, 1);
  A.at<double>(3, 2) = (undistorted_r.at<double>(1, 0) / undistorted_r.at<double>(2, 0)) * mat_P_r.at<double>(2, 2) -
                       mat_P_r.at<double>(1, 2);
  b.at<double>(0, 0) = -((undistorted_l.at<double>(0, 0) / undistorted_l.at<double>(2, 0)) * mat_P_l.at<double>(2, 3) -
                         mat_P_l.at<double>(0, 3));
  b.at<double>(1, 0) = -((undistorted_l.at<double>(1, 0) / undistorted_l.at<double>(2, 0)) * mat_P_l.at<double>(2, 3) -
                         mat_P_l.at<double>(1, 3));
  b.at<double>(2, 0) = -((undistorted_r.at<double>(0, 0) / undistorted_r.at<double>(2, 0)) * mat_P_r.at<double>(2, 3) -
                         mat_P_r.at<double>(0, 3));
  b.at<double>(3, 0) = -((undistorted_r.at<double>(1, 0) / undistorted_r.at<double>(2, 0)) * mat_P_r.at<double>(2, 3) -
...
void FrameHistory::image_callback(const sensor_msgs::ImageConstPtr &image_msg,
