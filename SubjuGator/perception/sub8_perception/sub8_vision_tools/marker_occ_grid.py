...
class OccGridUtils(object):
    ...
    def __init__(self, res, width, height, starting_pose, topic_name='/search_grid'):
        ...
        self.occ_grid_pub = rospy.Publisher(
            topic_name, OccupancyGrid, queue_size=1)
    ...
    def add_circle(self, center, radius):
        ...
        center_offset = np.array(
            center) / self.meta_data.resolution - np.array([self.mid_x, self.mid_y])
    ...
    def found_marker(self, pose_2d):
        ...
        center = np.array([pose_2d.x, pose_2d.y])
        rotation = -pose_2d.theta

        center_offset = center / self.meta_data.resolution - \
            np.array([self.mid_x, self.mid_y])

        rot_top_point = np.dot(
            np.array([TRENCH_LENGTH, 0]) / 2, make_2D_rotation(rotation)).astype(np.int32)
        rot_bottom_point = np.dot(-np.array([TRENCH_LENGTH, 0]) / 2,
                                  make_2D_rotation(rotation)).astype(np.int32)

        pos_top_point = np.int0(rot_top_point + center_offset)
        pos_bottom_point = np.int0(rot_bottom_point + center_offset)

        cv2.line(self.markers, tuple(pos_top_point), tuple(
            pos_bottom_point), 101, int(TRENCH_WIDTH))
...
class Searcher():
    ...
    def return_pose(self, srv):
        ...
        pose = msg_helpers.numpy_quat_pair_to_pose(
            coor, np.array([0, 0, 0, 1]))
    ...
    def check_searched(self, search_center, search_radius):
        ...
        '''
        center_offset = np.array(search_center) / \
            self.grid_res - self.position_offset

        # Create mask
        circle_mask = np.zeros(self.searched_area.shape)
        radius = int(search_radius / self.grid_res)
        cv2.circle(circle_mask, tuple(
            center_offset.astype(np.int32)), radius, 1, -1)
...
class MarkerOccGrid(OccGridUtils):
    ...
    def add_marker(self, marker, timestamp):
        ...
        trans, rot = self.tf_listener.lookupTransform(
            "/map", "/downward", timestamp)
        cam_rotation = tf.transformations.euler_from_quaternion(rot)[
            2] + np.pi / 2
        ...
        magnitude = self.calculate_visual_radius(
            height, second_point=marker[0])
...
if __name__ == "__main__":
    ...
    tr = MarkerOccGrid(res=.1, width=100, height=500,
