...
{
ODOM_ESTIMATOR_DEFINE_MANIFOLD_BEGIN(Odom, (ros::Time, stamp)(std::string, frame_id)(std::string, child_frame_id),

                                     (Vec<3>, pos)(QuaternionManifold, orient)(Vec<3>, vel)(Vec<3>, ang_vel))
ODOM_ESTIMATOR_DEFINE_MANIFOLD_END()

template <typename ArgType, typename ResType>
ResType functional(void func(ArgType const &, ResType &), ArgType const &arg)
{
  ResType res;
  func(arg, res);
  return res;
...
{
  return GaussianDistribution<Odom>(Odom(msg.header.stamp, msg.header.frame_id, msg.child_frame_id,
                                    ...
                                    (SqMat<12>() << Eigen::Map<const SqMat<6> >(msg.pose.covariance.data()),
                                     SqMat<6>::Zero(), SqMat<6>::Zero(),
                                     Eigen::Map<const SqMat<6> >(msg.twist.covariance.data()))
                                        .finished());
}
