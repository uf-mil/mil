...
int get_color(cv::Scalar color){
  ...
  auto b = color[1];
  auto c = color[2];
  if(fabs(a-b) < 10 && b > c && a > c) return 1;
  else if(a > b && a > c) return 2;
  else if(b > a && b > c) return 3;
  else if(c > a && c > b) return 4;
  else return 0;

}


bool ColorTracker::track(cv::Mat frame_left, std::vector<cv::Point2f> points_small, double image_proc_scale){
  ...
  if(started){
    ...
    if(col == 0){
      ...
      if(col == 1) colors[colors_found-1] = "y";
      else if(col == 2) colors[colors_found-1] = "b";
      else if(col == 3) colors[colors_found-1] = "g";
      else if(col == 4) colors[colors_found-1] = "r";
...
bool ColorTracker::check_status(navigator_msgs::ScanTheCodeMission::Request &req,
  ...
  mycolors.push_back(colors[0]);mycolors.push_back(colors[1]);mycolors.push_back(colors[2]);
