...
from subjugator_sim_tools import physics, rendering
from subjugator_sim_tools.meshes import Transdec
from subjugator_sim_tools.physics import Box, Mesh, Sphere
from subjugator_sim_tools.widgets import Sub
...
class SimWorld(rendering.Canvas):
    def __init__(self):
        ...
        self.physics_dt = rospy.get_param("physics_dt", 1 / 100.0)
        ...
        self.physics_world.add_entity(Mesh, (0.0, 0.0, 0.0), 10.0, Transdec)
        ...
        self.view = np.array(
            [
                [1.0, +0.000, +0.0000, +0.0],
                [0.0, +0.500, -0.8660, +0.0],
                [0.0, +0.866, +0.5000, +0.0],
                [0.0, -1.685, -36.931, +1.0],
            ]
        )
    ...
    def add_sub(self, position):
        self.sub = Sub(
            self.rendering_world, self.physics_world, position, thrust_indicators=False
        )
...
if __name__ == "__main__":
    sim = SimWorld()
    sphere = sim.add_sphere((-3.0, -3.0, 15.0), 4.2, 1.0, (255.0, 0.0, 0.0))
