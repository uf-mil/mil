...
class HSVCalibration:
    ...
    def reconfigure(self, config, level):
        try:
            ...
            rospy.logwarn('HSV lower bound below minimum value') if (self.lower < 0).any() else None
            self.upper = np.array(self.parse_string(config['dyn_upper']))
            rospy.logwarn('HSV upper bound above maximum values') if (self.upper[0] > 179).any() and (self.upper[1:] > 255).any() else None
