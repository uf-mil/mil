...
class ScanTheCodeAction(object):
    ...
    def initial_position(self, scan_the_code, distance=8):
        ...
        position = [scan_the_code.position.x,
                    scan_the_code.position.y, scan_the_code.position.z]
        now = datetime.datetime.now()
        now_time = now.time()
        if now_time < datetime.time(12, 00):
            return [position[0] - self.distance,
    ...
    def correct_pose(self, scan_the_code, pose):
        ...
        stc_position = np.array(
            [scan_the_code.position.x, scan_the_code.position.y, scan_the_code.position.z])
        d = np.linalg.norm(position - stc_position)
        point_a = position[0] + d * np.sqrt(3)
        dir_vec = (stc_position - point_a) / \
            np.linalg.norm(stc_position - point_a)
