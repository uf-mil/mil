...
public:
  ...
  bool check_point(Eigen::Vector3d point, cv::Point point2d, cv::Mat img,
                   cv::Matx34d left_cam_mat, bool check);
  void remove_point(Eigen::Vector3d point, cv::Point point2d);
  bool get_model(std::vector<Eigen::Vector3d> &model3d,
                 std::vector<cv::Point> &model_position_2d, cv::Mat img,
                 cv::Matx34d left_cam_mat);
  bool complete();
  void clear();
...
private:
  ...
  void visualize_points(std::vector<Eigen::Vector3d> feature_pts_3d,
                        ...
                        std::string name, double min_cost);
};
