...
/////////////////////////////////////////////////
PlacardPlugin::PlacardPlugin() : gzNode(new gazebo::transport::Node())
{
}

//////////////////////////////////////////////////
std_msgs::ColorRGBA PlacardPlugin::CreateColor(const double _r, const double _g, const double _b, const double _a)
{
...
//////////////////////////////////////////////////
void PlacardPlugin::Load(gazebo::rendering::VisualPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  if (this->shuffleEnabled)
  {
    ...
    this->changeSymbolSub = this->nh.subscribe(this->rosShuffleTopic, 1, &PlacardPlugin::ChangeSymbol, this);
  }

  this->nextUpdateTime = this->scene->SimTime();

  this->updateConnection = gazebo::event::Events::ConnectPreRender(std::bind(&PlacardPlugin::Update, this));

  this->gzNode->Init();
  this->symbolSub = gzNode->Subscribe(this->symbolSubTopic, &PlacardPlugin::ChangeSymbolTo, this, true);
}
...
bool PlacardPlugin::ParseSDF(sdf::ElementPtr _sdf)
{
  ...
  if (_sdf->HasElement("shape"))
  {
    ...
    if (std::find(this->kShapes.begin(), this->kShapes.end(), aShape) != this->kShapes.end())
    {
      this->shape = aShape;
    }
    else
    {
      ROS_INFO_NAMED("PlacardPlugin", "incorrect [%s] <shape>, using random shape", aShape.c_str());
    }
  }
  ...
  else if (_sdf->HasElement("color"))
  {
    ...
    {
      ROS_INFO_NAMED("PlacardPlugin", "incorrect [%s] <color>, using random color", aColor.c_str());
    }
  }
...
void PlacardPlugin::ChangeSymbol(const std_msgs::Empty::ConstPtr &_msg)
{
  ...
  ROS_INFO_NAMED("PlacardPlugin", "New symbol is %s %s", this->color.c_str(), this->shape.c_str());
}
