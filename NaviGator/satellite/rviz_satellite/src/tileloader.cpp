...
static size_t replaceRegex(const boost::regex &ex, std::string &str, const std::string &replace)
{
...
TileLoader::TileLoader(const std::string &service, double latitude, double longitude, unsigned int zoom,
                       unsigned int blocks, QObject *parent)
  : QObject(parent), latitude_(latitude), longitude_(longitude), zoom_(zoom), blocks_(blocks), object_uri_(service)
{
  assert(blocks_ >= 0);
  ...
  cache_path_ = QDir::cleanPath(QString::fromStdString(package_path) + QDir::separator() + QString("mapscache") +
                                QDir::separator() + QString::number(hash_fn(object_uri_)));

  QDir dir(cache_path_);
  if (!dir.exists() && !dir.mkpath("."))
  {
    throw std::runtime_error("Failed to create cache folder: " + cache_path_.toStdString());
  }
...
void TileLoader::start()
{
  ...
  QObject::connect(qnam_.get(), SIGNAL(finished(QNetworkReply *)), this, SLOT(finishedRequest(QNetworkReply *)));
...
void TileLoader::latLonToTileCoords(double lat, double lon, unsigned int zoom, double &x, double &y)
{
  if (zoom > 31)
  {
...
void TileLoader::finishedRequest(QNetworkReply *reply)
{
  ...
  const std::vector<MapTile>::iterator it =
      std::find_if(tiles_.begin(), tiles_.end(), [&](const MapTile &tile) { return tile.reply() == reply; });
  if (it == tiles_.end())
  {
  ...
  {
    const QString err = "Failed loading " + request.url().toString() + " with code " + QString::number(reply->error());
    emit errorOcurred(err);
  }
...
{
  const bool loaded = std::all_of(tiles_.begin(), tiles_.end(), [](const MapTile &tile) { return tile.hasImage(); });
  if (loaded)
  {
...
QUrl TileLoader::uriForTile(int x, int y) const
{
  ...
  replaceRegex(boost::regex("\\{x\\}", boost::regex::icase), object, std::to_string(x));
  replaceRegex(boost::regex("\\{y\\}", boost::regex::icase), object, std::to_string(y));
  replaceRegex(boost::regex("\\{z\\}", boost::regex::icase), object, std::to_string(zoom_));
...
{
  return "x" + QString::number(x) + "_y" + QString::number(y) + "_z" + QString::number(z) + ".jpg";
}

QString TileLoader::cachedPathForTile(int x, int y, int z) const
{
  return QDir::cleanPath(cache_path_ + QDir::separator() + cachedNameForTile(x, y, z));
}
