...
class Model6SV:
    ...
    def __init__(
        ...
        z_time: datetime = datetime.strptime(
            "2021-05-26 04:00", "%Y-%m-%d %H:%M"
        ),  # Zulu datetime
        station_num: int = 94299,  # radiosonde station number
        region: str = "pac",  # radiosonde region code
        alt: float = 0.12,  # altitude in km
        zen: float = 0.0,  # viewing zenith angle in degrees
        azi: float = 0.0,  # viewing azimuth angle in degrees
        ...
        if wavelength_array is None:
            wavelength_array = np.arange(400, 800, 4)
        ...
        z_hour = 0 if ((z_time.hour + int(lon / 30)) % 24 - 12) / 12.0 < 0.5 else 12
        ...
        s.atmos_profile = SixSHelpers.Radiosonde.import_uow_radiosonde_data(
            radiosonde_url, AtmosProfile.MidlatitudeSummer
        )
        ...
        if tile_type is not None:
            s.ground_reflectance = GroundReflectance.HomogeneousLambertian(tile_type)
    ...
    def __call__(self):
        ...
        self.radiance = self.run_wavelengths(
            self.wavelength_array
        )  # units of (W/m^2/sr/μm)

        df = pd.DataFrame(
            {"wavelength": self.wavelength_array, "radiance": self.radiance}
        )
    ...
    def show(self, plot_lib: str = "bokeh") -> "figure object":
        ...
        return hv.Curve(
            zip(self.wavelength_array * 1e3, self.radiance / 10),
            label="computed radiance",
        ).opts(xlabel="wavelength (nm)", ylabel="radiance (μW/cm$^2$/sr/nm)")
...
@patch
def run_wavelengths(
    self: Model6SV, wavelengths: np.array, n_threads: int = None
) -> np.array:
...
class SpectralLibrary:
    ...
    def __init__(self, speclib_path: str = None):
        ...
        if speclib_path is not None:
            if "gzip" in speclib_path:
                self.speclib = pd.read_parquet(speclib_path)
            else:
                self.speclib = pd.read_pickle(speclib_path)
            self.wavelengths = self.speclib["wavelength"].to_numpy()
        else:
            self.wavelengths = np.arange(350, 2501)
            self.speclib = pd.DataFrame(
                self.wavelengths.reshape(2151, 1), columns=["wavelength"]
            )
            warnings.warn(
                "Spectral Library file required! Please re-initialised with the file.",
                stacklevel=2,
            )
    ...
    def show(
        ...
        if plot_lib == "bokeh":
            ...
            plot_opts = {"fig_inches": (12, 5)}

        # plot all the reflectance spectra in the library
        if topk is None:
            curve_list = [
                hv.Curve(zip(self.wavelengths, self.speclib[i]), label=i)
                for i in self.speclib.columns
            ]
            if hasattr(self, "last_spectra"):
                return (
                    hv.Overlay(curve_list)
                    * hv.Curve(
                        zip(self.wavelengths, self.last_spectra), label="last spectra"
                    )
                ).opts(
                    **plot_opts,
        ...
        if hasattr(self, "last_spectra") and hasattr(self, "rad_6SV"):
            if radiance_6SV is not None:
                curve_list.append(
                    hv.Curve(
                        zip(self.wavelengths, self.last_spectra), label="tap point"
                    )
                )
            else:
                curve_list.append(
                    hv.Curve(
                        zip(self.wavelengths, self.last_spectra / self.rad_6SV),
                        label="6SV estimate",
                    )
                )
        alphas = topk["score"].to_numpy()
        alphas = np.exp(alphas) / np.sum(
            np.exp(alphas)
        )  # softmax the transparency alpha to bring out the closest match
        ...
        if radiance_6SV is not None:
            return hv.Overlay(curve_list).opts(
                **plot_opts,
                ...
                ylim=(0, 1.1 * np.max(radiance_6SV) / 10),
    ...
    def interp(self, wavelengths: np.array):
        ...
        self.speclib = pd.concat(
            [pd.DataFrame({"type": "openhsi", "wavelength": wavelengths}), self.speclib]
        )
        self.speclib.set_index("wavelength", inplace=True)
        self.speclib.interpolate(
            method="cubicspline", axis="index", limit_direction="both", inplace=True
        )
    ...
    def dump(self, save_path: str = None):
        ...
        if save_path is not None:
            if "gzip" in save_path:
                self.orig_speclib.to_parquet(save_path, compression="gzip")
            else:
                self.orig_speclib.to_pickle(save_path, protocol=4)
            print(f"Updated your spectral library at {save_path}")
        else:
            if "gzip" in self.speclib_path:
                self.orig_speclib.to_parquet(self.speclib_path, compression="gzip")
            else:
                self.orig_speclib.to_pickle(self.speclib_path, protocol=4)
...
def import_USGS(self: SpectralLibrary, directory: str, sort: bool = False):
    ...
    for f in fnames:
        ...
        temp.loc[
            ~(temp[temp.columns[0]] > 0), temp.columns[0]
        ] = np.nan  # replace any negative reflectance with NaNs
    ...
    self.orig_speclib = self.orig_speclib.loc[
        :, ~self.orig_speclib.columns.duplicated()
    ]
    if sort:
        self.orig_speclib = self.orig_speclib.reindex(
            sorted(self.orig_speclib.columns),
            axis=1,
        )
...
class SpectralMatcher(SpectralLibrary):
    ...
    def show(self, plot_lib: str = "bokeh", is_rad: bool = False) -> "figure object":
        ...
        if is_rad and hasattr(self, "sim_df"):
            return super().show(
                plot_lib, topk=self.sim_df, radiance_6SV=self.rad_6SV * 10
            )
    ...
    def topk_spectra(self, spectrum: np.array, k: int = 5, refine=True):
        ...
        cosine_dist = (
            self.last_spectra
            @ self.spectra
            / (norm(self.last_spectra) * self.spectra_norm)
        )  # less than O(n^3)
        topk_idx = np.argpartition(cosine_dist, -k)[
            -k:
        ]  # linear time rather than n log n
        self.topk_idx = topk_idx[np.argsort(cosine_dist[topk_idx])][::-1]  # k log k

        if refine and k > 1:
            topk_spectra = (
                self.spectra[self.speclib.columns[self.topk_idx]].to_numpy().transpose()
            )
            ...
            self.sim_df = pd.DataFrame(
                {
                    "label": self.speclib.columns[self.topk_idx],
                    "score": cosine_dist[self.topk_idx] - residuals[subset_topk_idx],
                }
            )
            return self.sim_df
        ...
        self.sim_df = pd.DataFrame(
            {
                "label": self.speclib.columns[self.topk_idx],
                "score": cosine_dist[self.topk_idx],
            }
        )
        return self.sim_df
...
def topk_spectra(self: SpectralMatcher, spectrum: np.array, k: int = 5, refine=True):
    ...
    self.last_spectra = np.array(spectrum)
    cosine_dist = (
        self.last_spectra @ self.spectra / (norm(self.last_spectra) * self.spectra_norm)
    )  # less than O(n^3)
...
class ELC(SpectralMatcher):
    ...
    def __init__(self, nc_path: str, old_style: bool = False, **kwargs):
        ...
        self.RGB = self.dc.show("bokeh", robust=True).opts(
            height=250, width=1000, invert_yaxis=True, tools=["tap"], toolbar="below"
        )
        ...
        self.title_txt = pn.pane.Markdown(
            "**Interactive Empirical Line Calibrator**",
        )
        self.event_msg = pnw.StaticText(
            name="", value="Click export buttons to save desired reflectance datacubes."
        )
        self.export_ELC_button = pnw.Button(
            name="Export ELC datacube", button_type="primary", width=150, height=25
        )
        self.export_6SV_button = pnw.Button(
            name="Export 6SV datacube", button_type="primary", width=150, height=25
        )
    ...
    def __call__(self):
        ...
        return pn.Column(
            ...
            pn.Tabs(
                ("Reflectance", self.ref_curve),
                ("Radiance", self.rad_curve),
                dynamic=True,
            ),
    ...
    def setup_export_ELC(self):
        ...
        def click_func(event):
            ...
            self.event_msg.value = f"ELC datacube exported to directory {save_dir}. Button clicked {self.export_ELC_button.clicks} time(s)"
    ...
    def setup_export_6SV(self):
        ...
        def click_func(event):
            ...
            self.event_msg.value = f"6SV datacube exported to directory {save_dir}. Button clicked {self.export_6SV_button.clicks} time(s)"
    ...
    def setup_streams(self):
        ...
        self.box_stream = hv.streams.BoxEdit(
            source=self.boxes, num_objects=5, styles={"fill_color": "red"}
        )
    ...
    def setup_callbacks(self):
        ...
        def tap_radiance(x, y):
            ...
            return (
                self.show(is_rad=True)
                .opts(
                    legend_position="right",
                    legend_offset=(0, 20),
                    title=f'top match: {self.sim_df["label"][0]}, score: {self.sim_df["score"][0]:.3f}, position=({y:d},{x:d})',
                )
                .opts(
                    framewise=True,
                    xlabel="wavelength (nm)",
                    ...
                    height=400,
                    width=1000,
                    ylim=(0, np.max(self.spectra["spectralon"])),
                )
            )

        self.rad_curve = hv.DynamicMap(tap_radiance, streams=[self.posxy]).opts(
            shared_axes=False, height=250
        )
        ...
        def tap_reflectance(x, y):
            ...
            return (
                (
                    hv.Curve(zip(self.wavelengths, spectra), label="ELC estimate")
                    * self.show(is_rad=False)
                )
                .opts(
                    axiswise=True,
                    legend_position="right",
        ...
        self.ref_curve = hv.DynamicMap(tap_reflectance, streams=[self.posxy]).opts(
            shared_axes=False, height=250
        )
        ...
        def update_ELC(data):
            ...
            self.xx[:, 0] = x[:, 0, 0]
            self.xx[:, 1] = x[:, 1, 0]
            self.a_ELC = self.xx[:, 0]
            self.b_ELC = self.xx[:, 1]
            return hv.Curve([])
...
class DataCubeViewer:
    """Explore datacubes
    Optional key/val pair argument overrides (**kwargs)
    ylim
    ylabel
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.dc.load_nc(nc_path, old_style)
        self.RGB = self.dc.show("bokeh", robust=True).opts(
            height=int(1000 * img_aspect_ratio),
            width=1000,
            invert_yaxis=True,
            tools=["tap"],
            toolbar="below",
    ...
    def __call__(self):
        ...
        return pn.Column(
            self.title_txt, self.RGB * self.hit_dmap * self.ch_dmap, self.tap_curve
        )
    ...
    def setup_callbacks(self):
        ...
        def tap(x, y):
            ...
            x_slice = slice(
                np.max((x - self._bhalf[1], 0)),
                np.min((x - self._bhalf[1] + self.box_sz[1], self.dc.dc.data.shape[1])),
            )
            y_slice = slice(
                np.max((y - self._bhalf[0], 0)),
                np.min((y - self._bhalf[0] + self.box_sz[0], self.dc.dc.data.shape[0])),
            )
            c = np.mean(self.dc.dc.data[y_slice, x_slice, :], axis=(0, 1))
            # c = self.dc.dc.data[y,x,:]

            return hv.Curve(
                zip(self.dc.binned_wavelengths, c), label=f"tap point at ({x},{y}),"
            ).opts(xlabel="wavelength (nm)", ylabel=self.ylabelplot, ylim=self.ylim)

        self.tap_curve = hv.DynamicMap(tap, streams=[self.posxy]).opts(
            shared_axes=False, height=250, width=1000
        )
