...
class RectangleFinder(object):
    ...
    def get_rectangle(self, roi, debug=None):
        ...
        edges_more = cv2.Canny(gray, 30, 50, apertureSize=3)
        # debug.add_image(edges_more, "edges_more")
        # $$$$$$$$$$$$$$$$$$$$ DEBUG $s$$$$$$$$$$$$$$$$$$$$$
        ...
        debug.add_image(self.edges_1, "edges_circle", topic="first_lines")
        ...
        # If lines > 2 pick the two that are closest to the middle
        new_lines = sorted(new_lines, key=lambda x: x[3])

        def make_rec(xmin, ymin, xmax, ymax):
            return [(xmin, ymin), (xmax, ymin), (xmin, ymax), (xmax, ymax)]

        def draw_circle(img, xmin, ymin, xmax, ymax, color=(255, 255, 0)):
        ...
        debug.add_image(self.edges_3, "shit", topic="points_and")
        ...
        if len(new_lines) == 4:
            ...
            draw_circle(self.edges_3, xmin, ymin, xmax, ymax)
            return True, make_rec(xmin, ymin, xmax, ymax)
    ...
    # From these valid lines get the points of the rectangle th correspond to these
