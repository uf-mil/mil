...
bool SubjuGatorBuoyDetector::segment_buoy(cv::Mat &input_image, cv::Point &center,
                                          std::vector<sub::Contour> &output_contours, std::string &target_name)
{
  ...
  cv::findContours(image_thresh.clone(), contours, hierarchy, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE,
                   cv::Point(0, 0));
...
bool SubjuGatorBuoyDetector::request_buoy_position_2d(subjugator_msgs::VisionRequest2D::Request &req,
                                                      subjugator_msgs::VisionRequest2D::Response &resp)
{
  ...
  {
    ROS_ERROR("Requested buoy target (%s) had no color calibration on the parameter server", target_name.c_str());
    return false;
  }
