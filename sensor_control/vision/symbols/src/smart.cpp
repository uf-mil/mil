...
public:
  ImageSearcher() : n("dock_shape_filter") {
    ...
    allFoundShapesPublish = n.advertise<navigator_msgs::DockShapes>("/dock_shapes/filtered_shapes", 10);
  }
  ...
  void shapeChecker(const navigator_msgs::DockShapes &symbols) {
    if (!active) return;
    navigator_msgs::DockShapes pubSymbols;
    for (auto newShape = symbols.list.begin(); newShape != symbols.list.end();
      ...
      for (int i = 0; i < foundShapes.size(); i++) {
        if (foundShapes[i].sameType(*newShape)) {
          ...
          if (found) {
            navigator_msgs::DockShape shape;
            if (foundShapes[i].getAverageShape(shape))
              pubSymbols.list.push_back(shape);
          }
    ...
    allFoundShapesPublish.publish(pubSymbols);
  }
