...
class TorpedoOctogonFinder:
    ...
    def __init__(self):
        ...
        self.octogons = {}
        for color in ["red"]:
            self.octogons[color] = Octogon(color, debug_cv=self.debug_cv)
        if self.debug_cv:
            cv2.waitKey(1)
            self.debug_images = {}
        ...
        rospy.Service("~2D", VisionRequest2D, self.request_octogon)
        rospy.Service("~pose", VisionRequest, self.request_octogon3d)
    ...
    def toggle_search(self, srv):
        ...
        looking at frames for octogons.
        """
        if srv.data:
            rospy.loginfo("Octogon FINDER: enabled")
            self.enabled = True
        else:
            rospy.loginfo("Octogon FINDER: disabled")
            self.enabled = False

        return SetBoolResponse(success=True)

    def request_octogon(self, srv):
        """
        Callback for 2D vision request. Returns centroid
        of buoy found with color specified in target_name
        if found.
        """
        if not self.enabled or srv.target_name not in self.octogons:
    ...
    def request_octogon3d(self, srv):
        """
        Callback for 3D vision request. Uses recent observations of octogon
        specified in target_name to attempt a least-squares position estimate.
        """
        if srv.target_name not in self.octogons or not self.enabled:
            return VisionRequestResponse(found=False)
        octogon = self.octogons[srv.target_name]
        if octogon.est is None:
            return VisionRequestResponse(found=False)
        return VisionRequestResponse(
            pose=PoseStamped(
                header=Header(stamp=self.last_image_time, frame_id="map"),
                pose=Pose(position=Point(*octogon.est)),
            ),
            found=True,
        )
    ...
    def print_status(self, _):
        ...
        if self.enabled:
            rospy.loginfo(
                ...
                self.octogons["red"].status,
                self.octogons["green"].status,
                self.octogons["yellow"].status,
            )
    ...
    def find_buoys(self):
        ...
        """
        for buoy_name in self.octogons:
    ...
    def get_best_contour(self, contours):
        ...
        # if cv2.contourArea(contours_sorted[0]) < self.min_contour_area * self.image_area:
    ...
    def find_single_buoy(self, buoy_type):
        ...
        """
        assert (
            buoy_type in self.octogons
        ), f"Buoys_2d does not know buoy color: {buoy_type}"
        octogon = self.octogons[buoy_type]
        mask = octogon.get_mask(self.last_image)
        ...
        if cnt is None:
            octogon.clear_old_observations()
            octogon.status = f"{err} w/ {octogon.size()} obs"
            return
        ...
        if not self.sanity_check(center, self.last_image_time):
            octogon.status = "failed sanity check"
            return False
        ...
        octogon.add_observation(center, (np.array(t), R), self.last_image_time)

        observations, pose_pairs = octogon.get_observations_and_pose_pairs()
        if len(observations) > self.min_observations:
            octogon.est = self.multi_obs.lst_sqr_intersection(observations, pose_pairs)
            octogon.status = "Pose found"
            if self.debug_ros:
                self.rviz.draw_sphere(
                    octogon.est,
                    color=octogon.draw_colors,
                    scaling=(0.2286, 0.2286, 0.2286),
                    frame="map",
                    _id=octogon.visual_id,
                )
        else:
            octogon.status = f"{len(observations)} observations"
