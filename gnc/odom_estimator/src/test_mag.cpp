...
int main(int argc, char** argv) {
  ...
  std::cout << 1e9*(enu_from_ecef_mat(pos_ecef) * ecef_from_inertial(t,
    mm.getField(inertial_from_ecef(t, pos_ecef), t))).transpose() << std::endl;
  std::cout << Vec<3>(15731.8, 5713.6, 53184.3).transpose() << std::endl;
  std::cout << 1e9*(enu_from_ecef_mat(pos_ecef) * ecef_from_inertial(t,
    mm.getFieldAndJacobian(inertial_from_ecef(t, pos_ecef), t).first)).transpose() << std::endl;
