#include "subjugator_gazebo/subjugator_thrusters.hpp"
...
void ThrusterPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  std::map<std::string, subjugator_gazebo::ThrusterDef> thrusterDefs;
  subjugator_gazebo::load_thrusters(thrusterLayout["thrusters"], thrusterDefs);
...
void ThrusterPlugin::ThrustCallback(const subjugator_msgs::Thrust::ConstPtr& thrust)
{
...
void ThrusterPlugin::OnUpdate()
{
  ...
  mtx.lock();
  std::vector<subjugator_msgs::ThrusterCmd> cmdBuffer;
