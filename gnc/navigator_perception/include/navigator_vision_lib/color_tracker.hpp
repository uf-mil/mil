...
class ColorTracker {
public:
  bool track(cv::Mat frame_left, std::vector<cv::Point2f> points,
...
private:
  ...
  ros::NodeHandle nh;
  ros::ServiceServer send_status = nh.advertiseService(
      "scan_the_code/mission_status", &ColorTracker::check_status, this);
  image_transport::ImageTransport image_transport =
      image_transport::ImageTransport(nh);

  image_transport::Publisher debug_image_color =
      image_transport.advertise("stereo_model_fitter/debug_img/color", 1, true);

  bool check_status(navigator_msgs::ScanTheCodeMission::Request &req,
