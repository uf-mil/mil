...
#define MY_BOOST_FUSION_ADAPT_STRUCT_FILLER_0(X, Y) ((X, Y)) MY_BOOST_FUSION_ADAPT_STRUCT_FILLER_1
#define MY_BOOST_FUSION_ADAPT_STRUCT_FILLER_1(X, Y) ((X, Y)) MY_BOOST_FUSION_ADAPT_STRUCT_FILLER_0
...
#define FOLD_LEFT_SKIPPING_FIRST(op, state, seq)                                                                       \
  BOOST_PP_IF(BOOST_PP_DEC(BOOST_PP_SEQ_SIZE(seq)), BOOST_PP_SEQ_FOLD_LEFT(op, state, BOOST_PP_SEQ_TAIL(seq)), state)
...
#define GENERATE_ARG(R, ATTRIBUTE_TUPLE_SIZE, I, ATTRIBUTE)                                                            \
                                                                                                                       \
  BOOST_PP_COMMA_IF(I)                                                                                                 \
  BOOST_PP_TUPLE_ELEM(ATTRIBUTE_TUPLE_SIZE, 0, ATTRIBUTE)                                                              \
  BOOST_PP_TUPLE_ELEM(ATTRIBUTE_TUPLE_SIZE, 1, ATTRIBUTE)

#define GENERATE_PARAM(R, ATTRIBUTE_TUPLE_SIZE, I, ATTRIBUTE) BOOST_PP_TUPLE_ELEM(ATTRIBUTE_TUPLE_SIZE, 1, ATTRIBUTE),

#define GENERATE_SETTER(R, ATTRIBUTE_TUPLE_SIZE, I, ATTRIBUTE)                                                         \
                                                                                                                       \
  BOOST_PP_COMMA_IF(I)                                                                                                 \
  BOOST_PP_TUPLE_ELEM(ATTRIBUTE_TUPLE_SIZE, 1, ATTRIBUTE)(BOOST_PP_TUPLE_ELEM(ATTRIBUTE_TUPLE_SIZE, 1, ATTRIBUTE))

#define GENERATE_SUBTRACTOR(R, ATTRIBUTE_TUPLE_SIZE, I, ATTRIBUTE)                                                     \
                                                                                                                       \
  BOOST_PP_COMMA_IF(I)                                                                                                 \
  BOOST_PP_TUPLE_ELEM(ATTRIBUTE_TUPLE_SIZE, 1, ATTRIBUTE) -                                                            \
      other.BOOST_PP_TUPLE_ELEM(ATTRIBUTE_TUPLE_SIZE, 1, ATTRIBUTE)

#define GENERATE_DEF(R, ATTRIBUTES_SEQ, I, ATTRIBUTE)                                                                  \
                                                                                                                       \
  unsigned int const BOOST_PP_CAT(BOOST_PP_TUPLE_ELEM(2, 1, ATTRIBUTE), _start) = BOOST_PP_IF(                         \
      I, BOOST_PP_CAT(BOOST_PP_TUPLE_ELEM(2, 1, BOOST_PP_SEQ_ELEM(BOOST_PP_DEC(I), ATTRIBUTES_SEQ)), _end), 0);        \
  unsigned int const BOOST_PP_CAT(BOOST_PP_TUPLE_ELEM(2, 1, ATTRIBUTE), _end) =                                        \
      BOOST_PP_CAT(BOOST_PP_TUPLE_ELEM(2, 1, ATTRIBUTE), _start) + BOOST_PP_TUPLE_ELEM(2, 1, ATTRIBUTE).rows();

#define GENERATE_ADDER(R, ATTRIBUTE_TUPLE_SIZE, I, ATTRIBUTE)                                                          \
                                                                                                                       \
  BOOST_PP_COMMA_IF(I)                                                                                                 \
  BOOST_PP_TUPLE_ELEM(ATTRIBUTE_TUPLE_SIZE, 1, ATTRIBUTE) +                                                            \
      other.segment(BOOST_PP_CAT(BOOST_PP_TUPLE_ELEM(ATTRIBUTE_TUPLE_SIZE, 1, ATTRIBUTE), _start),                     \
...
#define GENERATE_ASSERT(R, ATTRIBUTE_TUPLE_SIZE, ATTRIBUTE)                                                            \
                                                                                                                       \
  assert(BOOST_PP_TUPLE_ELEM(ATTRIBUTE_TUPLE_SIZE, 1, ATTRIBUTE) ==                                                    \
         other.BOOST_PP_TUPLE_ELEM(ATTRIBUTE_TUPLE_SIZE, 1, ATTRIBUTE));

#define ODOM_ESTIMATOR_DEFINE_MANIFOLD_IMPL(NAME, TYPE_ATTRIBUTES_SEQ, ATTRIBUTES_SEQ, ATTRIBUTE_TUPLE_SIZE)           \
                                                                                                                       \
  struct NAME                                                                                                          \
  {                                                                                                                    \
    BOOST_PP_SEQ_FOR_EACH_R(1, GENERATE_FIELD, ATTRIBUTE_TUPLE_SIZE, BOOST_PP_SEQ_TAIL(TYPE_ATTRIBUTES_SEQ))           \
                                                                                                                       \
    BOOST_PP_SEQ_FOR_EACH_R(1, GENERATE_FIELD, ATTRIBUTE_TUPLE_SIZE, BOOST_PP_SEQ_TAIL(ATTRIBUTES_SEQ))                \
                                                                                                                       \
    static int const RowsAtCompileTime = FOLD_LEFT_SKIPPING_FIRST(myop, 0, ATTRIBUTES_SEQ);                            \
                                                                                                                       \
    ...
    {                                                                                                                  \
      return FOLD_LEFT_SKIPPING_FIRST(myop2, 0, ATTRIBUTES_SEQ);                                                       \
                                                                                                                       ...
                                                                                                                       \
    Vec<RowsAtCompileTime> operator-(NAME const &other) const                                                          \
    {                                                                                                                  \
      BOOST_PP_SEQ_FOR_EACH_R(1, GENERATE_ASSERT, ATTRIBUTE_TUPLE_SIZE, BOOST_PP_SEQ_TAIL(TYPE_ATTRIBUTES_SEQ))        \
      return (Vec<RowsAtCompileTime>(rows()) << BOOST_PP_SEQ_FOR_EACH_I_R(                                             \
                  1, GENERATE_SUBTRACTOR, ATTRIBUTE_TUPLE_SIZE, BOOST_PP_SEQ_TAIL(ATTRIBUTES_SEQ)))                    \
          .finished();                                                                                                 \
    }                                                                                                                  \
      ...
      return NAME(                                                                                                     \
          BOOST_PP_SEQ_FOR_EACH_I_R(1, GENERATE_PARAM, ATTRIBUTE_TUPLE_SIZE, BOOST_PP_SEQ_TAIL(TYPE_ATTRIBUTES_SEQ))   \
              BOOST_PP_SEQ_FOR_EACH_I_R(1, GENERATE_ADDER, ATTRIBUTE_TUPLE_SIZE, BOOST_PP_SEQ_TAIL(ATTRIBUTES_SEQ)));  \
                                                                                                                       ...
                                                                                                                       \
    NAME(BOOST_PP_SEQ_FOR_EACH_I_R(1, GENERATE_ARG, ATTRIBUTE_TUPLE_SIZE,                                              \
                                   BOOST_PP_SEQ_TAIL(TYPE_ATTRIBUTES_SEQ) BOOST_PP_SEQ_TAIL(ATTRIBUTES_SEQ)))          \
      : BOOST_PP_SEQ_FOR_EACH_I_R(1, GENERATE_SETTER, ATTRIBUTE_TUPLE_SIZE,                                            \
                                  BOOST_PP_SEQ_TAIL(TYPE_ATTRIBUTES_SEQ) BOOST_PP_SEQ_TAIL(ATTRIBUTES_SEQ))            \
    {                                                                                                                  \
    }

#define APPEND_IF_SEQ_LENGTH_IS_1(SEQ, ITEM)                                                                           \
  BOOST_PP_IF(BOOST_PP_DEC(BOOST_PP_SEQ_SIZE(SEQ)), SEQ, BOOST_PP_SEQ_PUSH_BACK(SEQ, ITEM))

#define ODOM_ESTIMATOR_DEFINE_MANIFOLD_BEGIN(NAME, TYPE_ATTRIBUTES, ATTRIBUTES)                                        \
  ODOM_ESTIMATOR_DEFINE_MANIFOLD_IMPL(                                                                                 \
      NAME, BOOST_PP_CAT(MY_BOOST_FUSION_ADAPT_STRUCT_FILLER_0(0, 0) TYPE_ATTRIBUTES, _END),                           \
      APPEND_IF_SEQ_LENGTH_IS_1(BOOST_PP_CAT(MY_BOOST_FUSION_ADAPT_STRUCT_FILLER_0(0, 0) ATTRIBUTES, _END),            \
                                (Vec<0>, _dummy)),                                                                     \
      2)
...
ODOM_ESTIMATOR_DEFINE_MANIFOLD_BEGIN(ManifoldPair, , (First, first)(Second, second))
ODOM_ESTIMATOR_DEFINE_MANIFOLD_END()

    ODOM_ESTIMATOR_DEFINE_MANIFOLD_BEGIN(EmptyTestManifold, , ) ODOM_ESTIMATOR_DEFINE_MANIFOLD_END()
    ...
    // world frame.
    struct QuaternionManifold : public IManifold<QuaternionManifold, 3>,
                                public Quaternion
{
  QuaternionManifold(Quaternion q) : Quaternion(q.normalized())
  {
  }
  unsigned int rows() const
  {
...
public:
  AngleManifold(double angle) : angle(wrapAngle(angle))
  {
    assert(this->angle > -4);
    assert(this->angle < 4);
  }
