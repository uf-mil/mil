...
def ping_cb(processed_ping):
    ...
    print "Line_arr size: {}".format(line_array.shape)
    if abs(processed_ping.freq - target_freq) < freq_tol and processed_ping.amplitude > min_amp:
        ...
        try:
            ...
            print "shape before append: {}".format(line_array.shape)
            line_array = np.append(line_array, line_coeffs, 0)
            print "amplitudes shape: {}".format(sample_amplitudes.shape)
            sample_amplitudes = np.append(sample_amplitudes, processed_ping.amplitude)
            print "shape after append: {}".format(line_array.shape)
...
def visualize_line(p0, p1):
    ...
    marker.action = marker.ADD
    print "points type: {}".format(type(marker.points))
    ...
    marker.color.a = 1.0
    if pinger_position != Point(0, 0, 0):
        print "publishinga line marker!!!!!!!!!!!!!!!!!!!11"
        viz_pub.publish(marker)
