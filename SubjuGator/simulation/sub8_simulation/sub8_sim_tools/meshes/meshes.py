...
def load_and_cache_mesh(file_path: str, url=None):
...
def load_mesh(path):
    ...
    urls = {"Sub.obj": "http://goo.gl/f0ennf?gdriveurl"}
    ...
    if file_ in urls.keys():
        if os.path.exists(path + ".npz"):
            ...
            if "url" not in array_data.keys():
                print(
                    "SIM: Existing model does not contain a version for {}, downloading a new one...".format(
                        file_
                    )
                )
            ...
            # Yes - download a new copy
                ...
                print(
                    "SIM: Existing model out of date for {}, downloading a new one...".format(
                        file_
                    )
                )
                download_and_unzip(urls[file_], dir_)
    ...
    # It's not a file we downloaded, load it normally
        ...
        raise Exception(
            "SIM ERROR: Something went wrong, we could not load a mesh at {}".format(
                file_
            )
        )
