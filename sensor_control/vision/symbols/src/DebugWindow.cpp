...
void DebugWindow::UpdateResults(navigator_msgs::DockShapes& symbols) {	
  ...
  for (navigator_msgs::DockShape symbol : symbols.list) {
    ...
    cv::circle(res,Point(symbol.CenterX,symbol.CenterY),4,Scalar(255,255,255),5);
    putText(res, symbol.Shape + "(" + symbol.Color + ")", Point(symbol.CenterX-25, symbol.CenterY-25), 4, 1, Scalar(0,0,0),  3);
  }
