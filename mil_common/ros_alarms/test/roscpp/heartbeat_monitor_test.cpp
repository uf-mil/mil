...
TEST(HeartbeatMonitorTest, heartbeatMonitorTest)
{
  ...
  EXPECT_GE(1, hb_monitor.getNumConnections()) << "Monitor should be connected to at least one publisher";
  ...
  EXPECT_TRUE(hb_monitor.healthy()) << "Heartbeat should be healthy before the loss time threshold {time_to_raise} is "
                                       "cleared.";
  ...
  EXPECT_FALSE(hb_monitor.healthy()) << "Heartbeat shouldn't be healthy after {time_to_raise} w/o receiving a "
                                        "heartbeat.";
  listener.waitForUpdate();
  ...
  EXPECT_TRUE(listener.queryCleared()) << "Monitor has gotten beats for over {time_to_clear}, alarm should have "
                                          "cleared.";

  sleep_until(time_to_raise * 1.2, hb_monitor.getLastBeatTime());
  EXPECT_TRUE(listener.queryRaised()) << "Heartbeat shouldn't be healthy after {time_to_raise} w/o receiving a "
                                         "heartbeat.";
