...
from navigator_msgs.srv import MessageFollowPath
...
class GateClassifier:
    def __init__(self, mission: NavigationGatefinder, send_feedback):
    ...
    async def get_next_gate(self, boat_pose) -> Gate | int:
        ...
        # misssdaing red
            ...
            for db_object in objects:
                # Could we do this more efficiently with a different database request?
                self.send_feedback(db_object.labeled_classification)
                DB_BUOY = Buoy.from_perception_object(db_object)
                if db_object.labeled_classification == "red_cylinder":
                    if nearest_red:
        ...
        # missing green
            ...
            for db_object in objects:
                DB_BUOY = Buoy.from_perception_object(db_object)
                if db_object.labeled_classification == "green_cylinder":
                    if nearest_green:
        ...
        self.send_feedback(f"Midpoint: {mid_point}")

        # return next move point
        if mid_point and nearest_green and nearest_red:
            self.failures_to_move = 0
            self.send_feedback(f"nearest red: {nearest_red.id}")
            self.send_feedback(f"nearest green: {nearest_green.id}")
            self.trajectory.append(mid_point)
        ...
        self.send_feedback(f"Out of {len(objects)}, no gates were found...")
        return 0
    ...
    async def available_gates(self) -> list[Gate]:
        ...
        self.send_feedback(f"found {len(found_buoys)}...")
...
class NavigationGatefinder(NaviGatorMission):
    ...
    async def run(self, parameters):
        ...
        classifier = GateClassifier(self, self.send_feedback)
        ...
        while True:
            ...
            self.send_feedback(f"next_gate: {next_gate}")
            if isinstance(next_gate, Gate):
                self.send_feedback("Moving to a gate right now...")
                await self.move.set_position(next_gate.mid_pose[0]).look_at(
                    next_gate.mid_pose[0],
                ).go()
            elif next_gate == -1:
                self.send_feedback("All Done!")
                break
            # msg = MessageFollowPathRequest()
            # msg.entry_color = "R"
            # msg.finished = 1
        ...
        await self.set_classifier_enabled(SetBoolRequest(False))
