...
parser.add_argument(
    "input_file", help="name of the input yaml file specifying the racetrack"
)
# get output file name
parser.add_argument("output_file", help="location of output xacro file")
# get debug plot or not
parser.add_argument(
    "--debug", action="store_true", help="if present, will plot the points of the track"
)
...
def points_as_complex(pose, radius, center_dist, resolution):
    ...
    orientation = (
        complex(
            np.cos((90 - azimuth) * np.pi / 180), np.sin((90 - azimuth) * np.pi / 180)
        )
        * 1j
    )
...
if __name__ == "__main__":
    ...
    out += points_as_str(
        track["pose"], track["radius"], track["center_dist"], track["resolution"]
    )
    out += "    </road>\n"
    out += "  </xacro:macro>"
    out += "</world>\n"
