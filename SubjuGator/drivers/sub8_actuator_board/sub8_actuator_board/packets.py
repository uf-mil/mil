...
class InvalidAddressException(RuntimeError):
    ...
    def __init__(self, address: int):
        super().__init__(
            f"Attempted to command valve {address}, but valid addresses are only [0,{CommandMessage.NUM_VALVES - 1}]",
        )
...
class FeedbackMessage(ApplicationPacket):
    ...
    def __str__(self):
        return f"FeedbackMessage(address={self.address}, on={self.on})"
