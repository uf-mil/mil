...
AerialMapDisplay::AerialMapDisplay() : Display(), map_id_(0), scene_id_(0), dirty_(false), received_msg_(false)
{
  ...
  topic_property_ =
      new RosTopicProperty("LLA (PointStamped) Topic", "",
                           QString::fromStdString(ros::message_traits::datatype<geometry_msgs::PointStamped>()),
                           "LLA topic to subscribe to.", this, SLOT(updateTopic()));
  topic_property_2 = new RosTopicProperty("Odometry Topic", "",
...
void AerialMapDisplay::subscribe()
{
  ...
  if (!topic_property_->getTopic().isEmpty())
  {
    try
    {
      ...
      coord_sub_ = update_nh_.subscribe(topic_property_->getTopicStd(), 1, &AerialMapDisplay::llaCallback, this);
...
void AerialMapDisplay::llaCallback(const geometry_msgs::PointStamped &lla)
{
  // If the new (lat,lon) falls into a different tile then we have some
  // reloading to do.
  sensor_msgs::NavSatFix *msg = new sensor_msgs::NavSatFix();
  msg->latitude = lla.point.x;
  msg->longitude = lla.point.y;
  msg->altitude = lla.point.z;
