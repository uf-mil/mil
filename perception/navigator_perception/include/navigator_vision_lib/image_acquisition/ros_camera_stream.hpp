...
#include <image_acquisition/camera_frame_sequence.hpp>
#include <string>
#include <mutex>
#include <ros/ros.h>
...
public:
    ...
    using CamFramePtr = boost::shared_ptr<CamFrame>;
    using CamFrameConstPtr = boost::shared_ptr<CamFrame const>;
...
bool ROSCameraStream<img_scalar_t, float_t>::init(std::string &camera_topic)
{
    ...
    auto init_lambda = 
        ...
        {
            std::string init_msg {"Initializing ROSCameraStream with "};
            init_msg += camera_topic;
            ROS_INFO(init_msg.c_str());
    ...
    ROS_DEBUG("ROSCameraStream: Waiting for ROS time to become valid.");
    ros::Time::waitForValid(ros::WallDuration(5, 0));
