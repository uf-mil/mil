...
from sub8_alarm import alarm_handlers, meta_alarms, meta_alarms_inv, HandlerBase
import datetime
import inspect
import sys
...
class AlarmHandler(object):
    def __init__(self):
        ...
        for alarm in meta_alarms_inv:
           self.alarms[alarm] = Alarm(alarm_name=alarm, clear=True, header=sub8_ros_tools.make_header()) 

        self.alarm_republish_timer = rospy.Timer(rospy.Duration(0.05), self.republish_alarms)
        
        # Go through everything in the sub8_alarm.alarm_handlers package
