...
class PacificGate(SubjuGatorMission):
    ...
    async def run(self, args):
        ...
        # Turn toward gate from random orientation
        ...
        # Style Points
        yaw_angle = math.radians(720)
        pitch_angle = math.radians(720)
        roll_angle = math.radians(720)

        await self.go(
             self.move().yaw_left(yaw_angle)
        )

        await self.go(
            self.move().pitch_up(pitch_angle)
        )


        await self.go(
            self.move().roll_left(roll_angle)
        )
