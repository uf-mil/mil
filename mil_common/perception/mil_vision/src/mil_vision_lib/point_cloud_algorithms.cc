...
PcdColorizer::PcdColorizer(ros::NodeHandle nh, string input_pcd_topic, string output_pcd_topic, string rgb_cam_topic,
                           string rgb_cam_frame)
{
  this->nh = nh;
  ...
  rgb_cam_sub =
      img_transport.subscribeCamera(rgb_cam_topic, 10, [this](const sensor_msgs::ImageConstPtr &image_msg_ptr,
        ...
        if (!_intrinsics_set)
        {
          ...
          cam_intrinsics << K[0], K[1], K[2], K[3], K[4], K[5], K[6], K[7], K[8];
