...
from subjugator_msgs.msg import Thrust, ThrusterCmd
...
class ThrusterMapper:
    ...
    def __init__(self):
        ...
        which points to ``subjugator_thruster_mapper/config/thruster_layout.yaml``.
    ...
    def get_ranges(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        Returns:
            Tuple[np.ndarray, np.ndarray]: The minimum and maximum thrust for
            each thruster.
        """
        minima = np.array(
            [
    ...
    def get_b_matrix(self, srv: BMatrixRequest) -> BMatrixResponse:
        ...
        Returns:
            BMatrixResponse: The response message.
        """
        return BMatrixResponse(self.B.flatten())

    def generate_B(self, layout: dict) -> np.ndarray:
        """
        Construct the control-input matrix. Each column represents the wrench
        generated by a single thruster.
        ...
        Returns:
            np.ndarray: The control-input matrix.
        """
    ...
    def map(self, wrench: np.ndarray) -> tuple[np.ndarray, bool]:
        """
        ...
        Returns:
            tuple[np.ndarray, bool]: The thrusts for each thruster, and whether
            the mapping was successful.
        """
        # TODO: Account for variable thrusters
