...
class image_converter:
  ...
  def get_bounding_rect(self, contours):
    ...
    for i, cont in enumerate(contours):
      for j, _val in enumerate(cont):
        if(_val[0][0] < xmin): xmin = _val[0][0]
        if(_val[0][0] > xmax): xmax = _val[0][0]
        if(_val[0][1] < ymin): ymin = _val[0][1]
        if(_val[0][1] > ymax): ymax = _val[0][1]
  ...
  def geometry_test(self, fp):
    ...
    for i, p in enumerate(fp):
      ...
      if(val > .05): return False
    
    return True

  def get_salient_points(self, img, rect):
    if(rect[1]==rect[3] or rect[0]==rect[2]): return
  ...
  def get_rectangle(self, img, img_area, xmin_a, ymin_a):
    ...
    if(max_ar > 100):
      ...
      smear = 5
      xmin-=smear;ymin-=smear;xmax+=smear;ymax+=smear;
      if(xmin < 0):xmin=0
      if(ymin < 0):ymin=0
      if(xmax > w):xmax=w
      if(ymax > h):ymax=h
