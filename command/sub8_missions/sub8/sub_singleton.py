...
class PoseSequenceCommander(object):
    ...
    def go_to_sequence_eulers(self, positions, orientations):
        ...
        for i in xrange(len(positions)):
            yield self.sub.move.look_at_without_pitching(np.array(positions[i][0:3])).go()
            yield self.sub.move.relative(np.array(positions[i][0:3])).go()
            yield self.sub.move.set_orientation(
                transformations.quaternion_multiply(
                    self.sub.pose.orientation,
                    transformations.quaternion_from_euler(orientations[i][0], orientations[i][1], orientations[i][2]))).go()
