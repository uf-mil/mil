...
def viz_colorspaces(rgb_image, viz_scale, disp_size=None):
    ...
    w, h = (
        (int(rgb_image.shape[1] * viz_scale), int(rgb_image.shape[0] * viz_scale))
    ...
    images_and_labels = list(
        zip(
            [
                rgb_small,
                cv2.cvtColor(rgb_small, cv2.COLOR_BGR2HSV),
    ...
    if show_color and mouse_x is not None and mouse_y is not None:
        ...
        for i in range(len(images_and_labels)):
            ...
            text = (
                f"[{images_and_labels[i][1][0]}, {images_and_labels[i][1][1]}, {images_and_labels[i][1][2]}] = [{images_and_labels[i][0][mouse_y, mouse_x][0]:>3}   {images_and_labels[i][0][mouse_y, mouse_x][1]:>3}   {images_and_labels[i][0][mouse_y, mouse_x][2]:>3}]"
            )
