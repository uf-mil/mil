...
class Vrx(Navigator):
    ...
    def run_station_keeping(self):
        ...
        goal_msg = yield self.station_keep_goal.get_last_message()
    ...
    @txros.util.cancellableInlineCallbacks
    def wait_for_task_to_do(self, f):
        while True:
    ...
    def run_wayfinding(self):
        ...
        yield self.wait_for_task_to_do(lambda task: task.state in ['ready', 'running'])
        path_msg = yield self.wayfinding_path_sub.get_last_message()
        ...
        yield self.wait_for_task_to_do(lambda task: task.state in ['running'])
        for pose in poses:
