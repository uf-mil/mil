    ...
    def __init__(self, **kwargs):
        ...
        if callable(getattr(self, "get_temp", None)):
            self.cam_temperatures = CircArrayBuffer(
                size=(self.n_lines,), dtype=np.float32
            )
...
class SimulatedCamera(OpenHSI):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        def wavelength2xyz(λ):
            ...
            x̅ = (
                piecewise_Guass(λ, 1.056, 5998, 379, 310)
                + piecewise_Guass(λ, 0.362, 4420, 160, 267)
                + piecewise_Guass(λ, -0.065, 5011, 204, 262)
            )
            y̅ = piecewise_Guass(λ, 0.821, 5688, 469, 405) + piecewise_Guass(
                λ, 0.286, 5309, 163, 311
            )
            z̅ = piecewise_Guass(λ, 1.217, 4370, 118, 360) + piecewise_Guass(
                λ, 0.681, 4590, 260, 138
            )
            return np.array([x̅, y̅, z̅])

        self.λs = np.poly1d(
            np.polyfit(
                np.arange(len(self.calibration["wavelengths"])),
                self.calibration["wavelengths"],
                3,
            )
        )(np.arange(self.settings["resolution"][1]))
        self.xs = np.zeros((1, len(self.λs)), dtype=np.float32)
        self.ys = self.xs.copy()
        self.zs = self.xs.copy()
        for i in range(len(self.xs[0])):
            self.xs[0, i], self.ys[0, i], self.zs[0, i] = wavelength2xyz(self.λs[i])

        self.xyz_buff = CircArrayBuffer(
            self.settings["resolution"], axis=0, dtype=np.int32
        )
    ...
    def rgb2xyz_matching_funcs(self, rgb: np.ndarray) -> np.ndarray:
        ...
        for i in range(rgb.shape[0]):
            self.xyz_buff.put(
                rgb[i, 0] * self.xs + rgb[i, 1] * self.ys + rgb[i, 2] * self.zs
            )
        ...
        y = (
            ...
            / (
                np.exp(
                    ...
                    / (wavelengths * 1e-9 * BOLTZMAN_CONSTANT * T_K)
                )
                - 1
            )
        )
...
class ProcessRawDatacube(OpenHSI):
    ...
    def __init__(
        ...
        if hasattr(self.buff, "ds_temperatures"):
            self.get_temp = (
                lambda: -999
            )  # this function needs to exist to create temperature buffer
        super().__init__(
            n_lines=self.buff.dc.data.shape[1],
