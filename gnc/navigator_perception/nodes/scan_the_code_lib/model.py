...
class Model:
    ...
    def get_bounding_rect(self):
        ...
        for i, cont in enumerate(self.prev_points):
          for j, _val in enumerate(cont):
            if(_val[0] < xmin): xmin = _val[0]
            if(_val[0] > xmax): xmax = _val[0]
            if(_val[1] < ymin): ymin = _val[1]
            if(_val[1] > ymax): ymax = _val[1]
    ...
    def get_color(self,scalar):
        ...
        if(abs(blackness1) < 30 and abs(blackness2) < 30):
            ...
            print self.offset_sum, self.num_offset
            self.offset = [self.offset_sum[0] / self.num_offset, self.offset_sum[1] / self.num_offset, self.offset_sum[2] / self.num_offset,0]
            return 'k';

        if(self.offset is not None):
            scalar = np.add(scalar, self.offset)

        print "fixed_scalar", scalar

        a = scalar[0];
        b = scalar[1];
        c = scalar[2];
        if(abs(a-b) < 10 and (b-c) < 70 and b > c and a > c): return 'y';
        elif(a > b and a > c): return 'b';
        elif(b > a and  b > c): return 'g';
        elif(c > a and c > b): return 'r';
        else: return 'g';
