...
class TestSimIntegration(unittest.TestCase):
    def __init__(self, *args):
        ...
        for topic, function in zip(topics_cam_info, info_functions):
        ...
        for topic, function in zip(topics_image, image_functions):
    ...
    def test_odom(self):
        ...
        while (
            len(self.odom_pos_msg) < 3 or len(self.odom_ori_msg) < 4
    ...
    def test_absodom(self):
        timeout = rospy.Time.now() + rospy.Duration(1)
        while (
            len(self.absodom_pos_msg) == 0 or len(self.absodom_ori_msg) == 0
        ) and rospy.Time.now() < timeout:
            rospy.sleep(0.1)
        self.assertTrue(
    ...
    def verify_pos_ori(self, pos, initial_pos, ori, initial_ori, topic):
        # make assertions
        for actual, initial in zip(pos, initial_pos):
    ...
    def test_image(self):
        timeout = rospy.Time.now() + rospy.Duration(1)
        while (
            len(self.cam_info_msg) != 4 or len(self.cam_image_msg) != 4
        ) and rospy.Time.now() < timeout:
            rospy.sleep(0.01)
        self.assertTrue(
    ...
    def test_pointcloud(self):
        timeout = rospy.Time.now() + rospy.Duration(1)
        while (
            len(self.pc_info_msg) != 1 or len(self.pc_msg) != 1
        ) and rospy.Time.now() < timeout:
            rospy.sleep(0.01)
        self.assertTrue(
    ...
    def verify_info(self, res_info, initial_info, topic):
        for msg in res_info:
            for actual_dim, initial_dim in zip(msg, initial_info):
                self.assertNotEqual(
                    actual_dim,
                    initial_dim,
                    msg=(
