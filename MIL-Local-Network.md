...
**WARNING: No device running Windows is allowed to connect to this network unless explicitly allowed by one of the network administrators in the lab (Anthony or Daniel)!**

The network can easily be accessed by clients with a wired ethernet or a wireless connection. To connect over ethernet, simply plug the computer into a free port on the unmanaged switch, which is the same table as the mil-plumbusi server, or on the Sub8 Router, which is on the same table as the sub. To connect over WiFi, select the either the uf-sub-2ghz or uf-sub-5ghz network (uf-sub-5ghz is recommended if it is available on the machine) and ask a senior lab member for the password.
...
* Make sure that all of the ethernet and USB connections to the back are secure
* Boot the machine back up and wait a few minutes
* See if the issue has been fixed

If mil-plumbusi.mil.lan is not responding to pings:
* Try pinging it's ip, 192.168.1.1
* If this works, the DNS server is likely not responding to queries
* Further troubleshooting can be done through SSH now

If you are unable to get a DHCP lease from the server (i.e. you attempt to connect, but cannot):
* Try setting up a static connection with the ip=192.168.1.99, gateway=192.168.1.1, and netmask=255.255.255.0
* If you can now connect and ping the server, this means there is a problem with DHCP
* Further troubleshooting can be done through SSH now
...
If you want to ensure the security of the connection, you can have a look at this article for the server's key fingerprint. This should not be required on the LAN unless some nefarious business is going on. See Anthony for the password for this account or to have one made for you.
...
A few different IP ranges are configured as follows:
...
The "XX:XX:XX:XX:XX:XX" string represents the MAC address of **the connecting interface, not the ethernet interface**. This means that if the computer will be connecting to the network over WiFi, the MAC address of the WiFi interface is what needs to be set here. The "YY" string represents the IP that the client will be given and must be in the pre-defined range of for the device (e.g. a client computer's "YY" string needs to be between 30 and 89). The "hostname" string should be replaced with whatever the client computers hostname is. To find this out of a linux machine, run this:

    hostname

Multiple interfaces can be specified for the same client computer. For example, if one wants to connect over ethernet sometimes and WiFi other times, a *client-pc_ethernet* and a *client-pc_wireless* connection can be created. These will have the MAC addresses of their respective devices, but share the same IP address. While this is a useful trick, **connecting with both ethernet and WiFi will cause major network jankiness** and users should be careful to avoid this. My personal recommendation is to set a static ethernet IP address and use dynamic allocation for WiFi.

Please place new entries in the correct commented block and make sure that they are in descending numerical order.
...
Before adding any new hostnames to this file, **ensure that they are listed as static DHCP entries** (see above for details). They should also be present in the [scripts/update_hosts.sh](https://github.com/uf-mil/Sub8/blob/master/scripts/update-hosts.bash) file.

To add a new entry for hostname-based lookup, edit the /etc/bind/db.192 and add a line in descending numerical order:
...
To add a new entry for reverse lookup (this allows an IP query to resolve to a hostname), edit the /etc/bind/db.mil.lan and add a line in descending IP address order:
...
In order to use mil-plumbusi as a network time server, perform the steps listed below. Only use these if you understand what they does because this is not be necessary under normal circumstances.
...
Being the self-maintaining headless server that it is, mil-plumbusi has a built in service to automatically keep all of it's packages up to date, remove obsolete packages, and reboot when updates require it. This is facilitated by the unattended-upgrades package. Important files to note are /etc/apt/apt.conf.d/10periodic, which controls when and how the package is run, and /etc/apt/apt.conf.d/50unattended-upgrades, which controls which packages are updated and what to do after running.

The only thing that I can see being changed here is if a new repository has been added to /etc/apt/sources.list (or in /etc/apt/sources.list.d). This should really never happen unless a server is needed that is not in the main Ubuntu repositories. As an important side note, **this server is not meant to run ros, gazebo, or any MIL code**. It is only meant to be reliable network infrastructure.

The following configuration in /etc/apt/apt.conf.d/50unattended-upgrades is used to select which repository's packages are automatically upgraded:
...
There are two different methods configured for accessing the files on the server - SSHFS and Samba. If you wish to set up a new fileserver share, ask Anthony before diving in. He can do this in a couple minutes, but it might seem be fairly involved if you are not familiar with the process. In any case, the steps below will give you an idea as to how shares are managed as well as take you through the process of creating a new share.
...
A line should be added in /etc/rc.local inside of the block beginning with the comment "Link filesystem users to their data" for the share. If the user should have read and write access to the share, use this line:
...
This is all of the setup required for creating a new SSHFS share. After rebooting the server, you should easily be able to use sshfs, as documented [here](https://github.com/uf-mil/Sub8/wiki/Accessing-Bag-Files), to connect to the share with a password. You should also be able to copy your key file over to the server for ease of access, as was documented [here](https://github.com/uf-mil/Sub8/wiki/SSH-Keys).

Enabling samba access to the share is easy and it is highly recommended that each share be accessible over both fileservers unless a specific circumstance warrens that they not be. A configuration block for the new share needs to be added to the bottom of the /etc/samba/smb.conf file. The format for this is as follows:
...
The share should now be accessible over both SSHFS and Samba. This should also work remotely over SSH as detailed here. Enjoy your large file storage!