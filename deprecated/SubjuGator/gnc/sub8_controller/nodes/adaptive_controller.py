...
class AdaptiveController(Controller):
    ...
    def end(self):
        ...
        wrench_hist = np.vstack(
            map(lambda o: np.clip(o, [-250, -250, -250],
                                  [250, 250, 250]), self.wrench_history)
        )
    ...
    def execute(self, current, target, state_history):
        ...
        error_orientation = self.orientation_error(
            target_orientation_q, current_orientation_q)
        error_angular_vel = current_angular_vel - target_angular_vel

        theta_hat = self.adaptive_update(
            error_position, error_linear_vel, current_linear_vel)

        # kr = (self.cfg['kp_trans'] * error_position) + (self.cfg['kd_trans'] * -error_linear_vel)
        kr = self.Kp_trans.dot(error_position) + \
            self.Kd_trans.dot(error_linear_vel)

        # world_force = -(self.get_Y(error_linear_vel, current_linear_vel).dot(theta_hat) - kr)
        # world_force = -kr + np.array([0.0, 0.0, 99.1])
        world_force = -kr - \
            self.get_Y(error_linear_vel, current_linear_vel).dot(theta_hat)
