...
class PingDirection:
    ...
    Creates a node named ``ping_direction``.
    """

    def __init__(self):
        rospy.init_node("ping_direction")
        self.sub = rospy.Subscriber("pings", numpy_msg(Triggered), self.ping_cb)
        self.pub = rospy.Publisher("angle", Float64, queue_size=10)
    ...
    def enable(self, req: SetBoolRequest) -> SetBoolResponse:
        """
        Sets the direction to enabled. Doesn't immediately do anything.
    ...
    def ping_cb(self, msg) -> None:
        ...
        # print("cross_corr:", cross_corr.shape)
        ...
        if (maxes == cross_corr.shape[0] - 1).any() or (maxes == 0).any():
            rospy.logerr(
                "/hydrophones/ping_direction lack of features on one of the channels"
            )
            return
        ...
        try:
            print(deltas)
            theta = util.calculate_pinger_direction(deltas, self.dist_h, self.v_sound)
        except Exception as e:
            rospy.logwarn(
                "/hydrophones/ping_direction could not calculate pinger direction %s"
                % e
            )
            return
