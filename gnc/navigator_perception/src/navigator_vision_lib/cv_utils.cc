...
sensor_msgs::ImagePtr convert_to_ros_msg(std::string type, cv::Mat image){
  return cv_bridge::CvImage(std_msgs::Header(), type, image).toImageMsg();
}
...
void anisotropic_diffusion(const cv::Mat &src, cv::Mat &dest, int t_max)
{
    ...
    cv::Mat    dI00 = cv::Mat::zeros(x0.size(),CV_32F);
    ...
    while (t < t_max)
        {
            ...
            cv::Mat D; // defined just before equation (5) in text
            cv::Mat gradxX,gradyX; // Image Gradient t time
            Sobel(x0,gradxX,CV_32F,1,0,3);
            Sobel(x0,gradyX,CV_32F,0,1,3);
            D = cv::Mat::zeros(x0.size(),CV_32F);
            ...
            x1 = cv::Mat::zeros(x0.size(),CV_32F);
