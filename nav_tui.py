...
class nav_tui(object):
	...
	#Cb function for each of the thrusters' status
	def thruster_fault_cb(self, status, topic):
		...
		#Method for converting the 8-bit int fault value into a list of the corresponding faults																
	...
	def display(self):
		...
		while True:
			...
			self.window.addstr(0, (self.x - len(" NaviGator Diagnostics TUI ")) / 2, " NaviGator Diagnostics TUI ", curses.color_pair(4))

			self.window.addstr(2, 0, kill_name, color)
			#Displays red if the boat is killed, green otherwise
			if self.raised == True:
				self.window.addstr(3, 0, kill_raised, curses.color_pair(1))
			elif self.raised == False:
