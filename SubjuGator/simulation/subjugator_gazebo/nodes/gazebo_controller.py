...
from subjugator_gazebo.srv import ResetGazebo, ResetGazeboResponse
...
class GazeboInterface:
    ...
    def publish_height(self, msg):
        ...
        self.dvl_pub.publish(
            RangeStamped(header=mil_ros_tools.make_header(), range=msg.range)
        )
    ...
    def publish_blueview_ping(self, msg):
        self.blueview_ranges_pub.publish(
            BlueViewPing(
                ...
                ranges=msg.ranges,
                intensities=[
                    np.uint16(x) if not math.isinf(x) else 0 for x in msg.ranges
                ],
            )
        )
