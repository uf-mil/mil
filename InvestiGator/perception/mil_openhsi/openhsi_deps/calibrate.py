...
def sum_gaussians(
    x: "indices np.array", *args: "amplitude, peak position, peak width, constant"
) -> np.array:
    split = len(args) // 3
    ...
    return (
        np.array(
            [A[i] * np.exp(-np.square((x - mu[i]) / sigma[i])) for i in range(len(A))]
        ).sum(axis=0)
        + c
    )
...
class SettingsBuilderMixin:
    ...
    def update_row_minmax(
        ...
        print(f"Locs row_min: {locs[0]} and row_max: {locs[1]}")
        row_min = int(
            locs[0] + edgezone
    ...
    def fit_emission_lines(
        ...
        shifted_idxs = np.arange(self.settings["resolution"][1])[
            _start_idx : _start_idx + _num_idx
        ]
        ...
        plt.plot(
            sum_gaussians(np.arange(len(spectra)), *coeffs), "r:", label="curve fit"
        )
        plt.legend()
        ...
        # interactively confirm peak wavelengths
        ...
        if interactive_peak_id:
            ...
            for i, pk in enumerate(top_A_idx.tolist()):
                print(
                    f"Peak {i} at col {μ[pk]} - default wavelength {brightest_peaks[i]}:"
                )
                res = input()
                if res:
        ...
        plt.legend(["Identified Peaks", "Spectra"])
        plt.xlabel("array index")
        ...
        for i in range(len(diffs)):
            if verbose:
                print(f"difference HgAr_lines - λ = {diffs[i]}")
            if diffs[i] < max_match_error:  # nm
                closest_HgAr_line.append(
                    HgAr_lines[np.argmin(np.abs(HgAr_lines - predicted_λ[i]))]
                )
        ...
        # perform final fit of wavelength with paired lines and peaks.
        ...
        for λ in spec_wavelengths:
            plots_list.append(
                hv.Curve(
                    zip((λ, λ), (0, np.max(spectra))),
                ).opts(color="r", alpha=0.5)
            )
    ...
    def fit_HgAr_lines(
        ...
        return self.fit_emission_lines(
            brightest_peaks=brightest_peaks,
    ...
    def update_intsphere_fit(
        ...
        if show:
            ...
            ax.plot(
                wavelen_arr, self.calibration["sfit"](wavelen_arr), label="Spline Fit"
            )
            ax.grid("on")
            # plt.axis([393,827,0,200])
            ax.set_xlabel("wavelength (nm)")
            ax.set_ylabel(r"spectral radiance ($\mu$W/cm$^2$/sr/nm)")
            ax.legend()
            ax.axvspan(
...
class SettingsBuilderMetaclass(type):
    def __new__(cls, clsname: str, cam_class, attrs) -> "SettingsBuilder Class":
        ...
        return super().__new__(cls, clsname, (cam_class, SettingsBuilderMixin), attrs)


def create_settings_builder(
    clsname: str, cam_class: "Camera Class"
