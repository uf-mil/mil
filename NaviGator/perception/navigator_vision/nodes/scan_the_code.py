...
def bbox_countour_from_rectangle(bbox) -> np.ndarray:
    return np.array(
        [
            [bbox[0][0], bbox[0][1]],
            [bbox[1][0], bbox[0][1]],
...
class ScanTheCodePerception:
    ...
    def panel_points_cb(self, pc2: PointCloud2) -> None:
        rospy.loginfo_throttle(10.0, "BBox received")
        self.last_panel_points_msg = pc2

    def get_panel_bbox(self) -> Optional[np.ndarray]:
    ...
    def img_cb(self, img: Image) -> None:
        if not self.enabled:
            return
        if self.camera_model is None:
            return
        ...
        if bbox is not None:
            ...
            symbol = label[10]
            if (
                len(self.classification_list) == 0
                or self.classification_list[-1] != symbol
            ):
            ...
            if (
                len(self.classification_list) == 5
                and self.classification_list[0] == "o"
                and self.classification_list[4] == "o"
                and self.classification_list[1] != "o"
                and self.classification_list[2] != "o"
