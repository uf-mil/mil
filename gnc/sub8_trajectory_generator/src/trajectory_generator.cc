...
class TGenNode {
 public:
  ...
  bool findPath(sub8_msgs::PathPlan::Request& req,
                      sub8_msgs::PathPlan::Response& resp) {
    
    geometry_msgs::Pose start_state = req.start_state;
    geometry_msgs::Pose goal_state = req.goal_state;

    if (!(_tgen->setProblemDefinition(
            _tgen->poseToState(start_state),
            _tgen->poseToState(goal_state)))) {
      // throw an error here?
...
int main(int argc, char** argv) {
  ...
  sub8::trajectory_generator::TGenNodePtr tgen(
      new sub8::trajectory_generator::TGenNode(std::move(AlarmBroadcasterPtr(
  ...
  ros::ServiceServer motion_planning_srv = nh.advertiseService(
      "sub8_trajectory_generator/path_plan",
      &sub8::trajectory_generator::TGenNode::findPath, tgen);
