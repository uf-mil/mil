...
class XimeaCamera(OpenHSI):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.xiapi = xiapi  # make available for later access just in case.

        self.xicam = self.xiapi.Camera()

        self.xicam.open_device_by_SN(
            serial_num
        ) if serial_num else self.xicam.open_device()
...
class LucidCamera(OpenHSI):
    ...
    def __init__(self, **kwargs):
        ...
        self.arsys = arsys  # make available for later access just in case.
        ...
        self.deviceSettings["BinningHorizontal"].value = self.settings["binxy"][
            0
        ]  # binning is symmetric on this sensor, no need to set vertical
        ...
        self.deviceSettings[
            "ExposureAuto"
        ].value = "Off"  # always off as we need to match exposure to calibration data
    ...
    def set_exposure(self, exposure_ms: float):
        ...
        self.deviceSettings[
            "ExposureTime"
        ].value = exposure_us  # requires time in us float
    ...
    def get_mac(self) -> str:
        return ":".join(
            [
                f"{a}{b}"
                for a, b in zip(
                    *[iter("{:012x}".format(cam.deviceSettings["GevMACAddress"].value))]
                    * 2
                )
            ]
        )
...
def switched_camera(
    ...
    if "Shared" in cam_name:
        eval(f"exec('from openhsi.shared import {cam_name}')")
    else:
        eval(f"exec('from openhsi.cameras import {cam_name}')")
    cam = FlirCamera(
        n_lines=n_lines,
