    ...
    if len(sys.argv) == 3:
        ...
        with open(filename, 'rb') as f:
            ...
            for i in itertools.count():
                ...
                if len(d) != 8*samples_per_callback: break
...
def filter(sample_generator, h):
    ...
    for samples in sample_generator:
        ...
        res = apply_along_first_axis(lambda x: scipy.signal.fftconvolve(x, h), samples)
...
def _max_convolve_slow(x, n):
    assert n >= 1
    return [max(x[i:i+n]) for i in xrange(0, len(x) - n + 1)] # equivalent, but less efficient
...
def triggering(sample_generator):
    ...
    if 1: # bandpass
        h = scipy.signal.remez(201, [0, f_low-transition_width, f_low, f_high, f_high+transition_width, 600e3], [0, 1, 0], Hz=fs)
    else: # no bandpass
        h = numpy.array([1])
    if 1: # notch out 40kHz
        h = numpy.concatenate([h, numpy.zeros(21000-len(h))]) # tweak filter length so that we have a bin on exactly the desired frequency with the desired width
        H = numpy.fft.rfft(h)
    ...
    if 0: # change to 1 to view filter response
        h2 = numpy.concatenate([h, numpy.zeros(len(h)*9)]) # zero pad to get more frequency resolution
        freq, response = numpy.fft.rfftfreq(len(h2), 1/fs), numpy.abs(numpy.fft.rfft(h2))
        ...
        pyplot.show()
    sample_generator = filter(sample_generator, h) # comment me out to disable filter entirely
    ...
    for samples in sample_generator:
        ...
        while True:
            ...
            if processing_now[1] - processing_now[0] <= 0: break
            
            samples_with_extra = sample_buf[:, processing_now[0]-(QUIET_TIME + IGNORE_TIME):processing_now[1]]
            samples_without_extra = sample_buf[:, processing_now[0]:processing_now[1]]
            assert numpy.all(samples_with_extra[:, QUIET_TIME + IGNORE_TIME:] == samples_without_extra)
            
            trigger_mag = THRESHOLD*_max_convolve(numpy.abs(samples_with_extra[0]), QUIET_TIME + 1)[:-IGNORE_TIME]
            assert trigger_mag.shape == (samples_without_extra.shape[1],)
            #pyplot.plot(trigger_mag)
            #pyplot.plot(samples_without_extra[0])
            #pyplot.show()
            triggered = numpy.nonzero(numpy.abs(samples_without_extra[0]) >= trigger_mag)[0] + processing_now[0]
            for y in triggered:
            ...
            if len(triggered):
                ...
                this_ping = sample_buf[:, first_sample-BEFORE:first_sample+AFTER].copy()
                this_trigger_mag = sample_buf[:, first_sample-BEFORE:first_sample+AFTER].copy()
