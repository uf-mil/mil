...
class ImageHandler:
    def __init__(self, filepath, savepath, bag):
        ...
        if savepath is not None:
            ...
            if savepath[-1] != "/":
                savepath += "/"
            self.working_dir = savepath + self.bagname.split(".")[0] + "_images"
        else:
    ...
    def parse_bag(self):
        ...
        for i in range(0, len(bag.get_type_and_topic_info()[1].values())):
            ...
            bag_msg_cnt.append(list(bag.get_type_and_topic_info()[1].values()[i][1]))

        topics = list(zip(topics, types, bag_msg_cnt))
    ...
    def save_images(self):
        ...
        with tqdm(total=self.msgs) as pbar:
            for topic, msg, t in self.bag.read_messages(topics=self.image_topics):
                try:
                    cv_image = self.bridge.imgmsg_to_cv2(msg, "bgr8")
                    cv2.imwrite(
                        self.working_dir + "/" + str(self.image_index) + ".png",
                        cv_image,
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f", "--filepath", help="File path containing the ROS bags")
    parser.add_argument("-s", "--savepath", help="Path where the images will be saved")
    parser.add_argument("-r", "--resize", help="Resolution to resize images to")
